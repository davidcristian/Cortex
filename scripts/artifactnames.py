...
**A settings field is read for what its own name says it holds, not only for where it is spent.**
A tier's `model_path` is one keyword of several that carry an artifact into an argv, and the
multimodal projector rides the cortex tier's `extra` instead, assembled by a call the tier reader
deliberately refuses to approximate. So the hosted side is read a second way, off the field names
...
def files(module: ast.Module) -> tuple[tuple[str, str, int], ...]:
    ...
    The alias walk underneath decides what a field is named in the environment and refuses a
    settings class that names nothing; this adds only the two things that walk drops, which field
    a name belongs to and where it is written.
    """
