...
holding an importable tree), because the walk finds a logger by the name a document prints and that
name is a function of where the module sits. The last test reads the committed brain, so the
spellings the fixtures exercise are held to a tree this repo really ships.
"""
...
# this file spells: which sinks are self-named is read out of the tree rather than listed here.
# The constant registry ties this name to the sinks that write it, so the guard cannot be deleted
# unnoticed and a sink cannot rename its declaration away from the guard asking for it.
...
def test_a_sink_that_names_itself_is_found_under_the_name_it_chose(tmp_path: Path) -> None:
    ...
    It stays supported because ``getLogger("name")`` is legal Python a brain module may write
    tomorrow, and a reader that stopped matching it would drop that logger out of the answer
...
def test_a_sink_naming_its_logger_through_a_constant_is_found_under_that_name(
    ...
    """This is the recall trail's spelling. Its name is a declaration because three documents
    restate it and the constant registry ties them to it, so a reader that matched only a literal
    would drop that trail out of this answer and report a sample of it as a logger no module
    declares."""
    brain(
        tmp_path,
        {
...
def declarations(root: Path) -> dict[str, str]:
    ...
    This walks the tree ``loggers`` already walks, deliberately: the guard below compares the two
    answers, so a walk here that stopped finding modules comes back empty against a set that is
    not, rather than agreeing with itself about a tree neither of them read.
    """
    found: dict[str, str] = {}
...
def self_named(root: Path) -> dict[str, str]:
    ...
    This is read off the call rather than off any declaration: ``loggers`` answers with the name
    the call carries, in whichever of the three spellings, so a name that is not the dotted path
    of the module writing it belongs to a sink that chose its own.
    """
    found: dict[str, str] = {}
...
def test_every_self_named_sink_binds_the_name_its_own_call_is_handed() -> None:
    ...
    because its lines are read as a trail, which is why documents restate that name, and why the
    name is a declaration for the constant registry to tie them to. Neither of those says the sink
    passes what it declares: ``getLogger(_LOGGER_NAME)`` carries an identifier, so a module binding
    one name and passing another holds two names rather than one name spelled twice, which is the
    shape the one-name rule allows through (ADR-0009 declared-name addendum).

    Comparing the two readings as sets holds every direction of that at once, and holds them for a
    sink written tomorrow as well as for the two written down. A sink that passes another name is a
    self-named logger this brain declares nowhere; one that declares a name it stops passing is a
    declaration the documents are still tied to and nothing writes; one that names itself with a
    bare literal has no declaration to tie them to at all; and one that binds its name under some
    other identifier is outside the naming the left-hand set is read by, which is the shape a
    derived rule has to hold rather than assume (ADR-0029 addendum on the naming a derived set is
    read out of). Each is one set carrying a pair the other does not.

    The non-emptiness beside it keeps the comparison from passing on two empty answers, and it
    also guards the fixtures above: a brain where no sink named itself would make both of the
    spellings they exercise fiction.
    """
