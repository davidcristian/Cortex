...
FIGURES = "Summarize the report below, keeping its figures."
...
def test_the_second_summarization_is_its_own_shape() -> None:
    declared = envelopejudges.declared(FIGURES)
    assert declared is not None
    assert declared.shape == "Summarize the report below, keeping its figures"
...
def without(text: str, *parts: str) -> str:
    """Return ``text`` with every one of ``parts`` removed."""
    for part in parts:
...
def test_a_copy_delivers_nothing_on_any_declared_shape() -> None:
    for ask in (SUMMARIZE, FIGURES, EXTRACT, LOOKUP):
