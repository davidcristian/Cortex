...
def test_a_variable_that_only_starts_like_the_subagent_prefix_is_not_a_subagent_model() -> None:
    """`CORTEX_MODEL_FILE_EMBED` and `CORTEX_MODEL_FILE_CORTEX` are the two neighbours, both in the
    family every artifact here is named in and neither beginning with the longer prefix that says
    a server is serving this tier.
    """
    cortex = (
