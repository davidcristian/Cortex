...
EVERY_RULE_CASES: list[tuple[str, ci_paths.Verdict]] = [
    # BOTH: shared gate files affect every toolchain.
    ("justfile", ci_paths.BOTH),
    (".python-version", ci_paths.BOTH),
    ("proto/body.proto", ci_paths.BOTH),
    ("scripts/linecap.py", ci_paths.BOTH),
    (".github/workflows/ci.yml", ci_paths.BOTH),
    # PYTHON only.
...
PRECEDENCE_CASES: list[tuple[str, ci_paths.Verdict]] = [
    ...
    ("scripts/README.md", ci_paths.BOTH),
    ("proto/README.md", ci_paths.BOTH),
    # The workflows prefix beats the `.md`/unmatched fallthroughs.
    (".github/workflows/release.yml", ci_paths.BOTH),
...
def test_classify_fails_closed_to_both_for_unmatched_paths(path: str) -> None:
...
def test_main_logs_one_verdict_line_per_path_on_stderr(
    ...
    assert captured.err == (
        ...
        "ci-paths: docker-compose.yml -> both (fail-closed default)\n"
    )
