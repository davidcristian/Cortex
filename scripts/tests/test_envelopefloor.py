...
def turn(
    *,
    ...
    """Return one run as the driver writes it, by default one that succeeded and answered."""
    return {
        "question": "warehouse",
...
def runs(
    ...
    *,
    ...
    """Return ``count`` identical runs, as the driver would have written them."""
...
def cell(count: int, of: int) -> tuple[Turn, ...]:
    """Return ``of`` runs, ``count`` of which answered, the rest repeating the instruction back."""
