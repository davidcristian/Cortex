...
def test_an_unmarked_tail_the_key_changed_publishes_nothing(tmp_path: Path, plain: str) -> None:
    """This is the state the reader will not guess at."""
...
def test_a_control_under_an_unmarked_tail_names_both_readings(
    ...
    thought with a system turn at the front, and one closing its thought with a marker this
    reader does not list, rendered the same both ways so the tail comparison passes it.
    """
    printed, code = report(
        sample(
            tmp_path / "s.json",
