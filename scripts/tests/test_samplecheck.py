...
def repo(root: Path, *, module: str = SETTLE, runbook: str = RUNBOOK) -> Path:
    """Write a small repo with one logging module and a runbook that shows its log line."""
    source = root / MODULE
...
def audited(
    ...
    *,
    ...
    """Write a small repo whose one log line is the audit's, beside the suite that asserts it."""
...
def detail(root: Path) -> str:
    """Return the single miss the fixture produces, asserting the count so a miscount fails."""
