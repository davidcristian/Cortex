...
def _repo(root: Path) -> Path:
    """Write both backlog directories, each with one task file and an index with an empty block."""
...
def test_check_links_reads_the_text_it_is_handed_and_not_the_file(tmp_path: Path) -> None:
...
def test_main_checks_the_index_links_on_the_text_the_write_run_puts_on_disk(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    root = _repo(tmp_path)
    _write(root, "docs/refinements/tasks/002-a-sibling.md", REFINEMENT)
    task = root / REFINEMENTS / "tasks" / "001-wire-the-memory-port.md"
    waiting = REFINEMENT.replace("open, actionable", "open, waiting for its trigger")
    task.write_text(waiting + "**Trigger:** [the sibling](002-a-sibling.md) is added.\n")
    ...
    task.write_text(waiting + "**Trigger:** the sibling is added.\n")
    assert backlogcheck.main(["--root", str(root), "--write"]) == 0
...
def test_main_checks_no_pointer_at_a_backlog_whose_index_it_could_not_render(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    root = _repo(tmp_path)
    _write(root, "docs/adr/ADR-0001.md", "See [the area](../refinements/index.md#brain).\n")
