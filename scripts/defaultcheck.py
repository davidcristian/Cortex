...
The rule holds one variable's several defaults to the same **value** rather than to the same
text, compared through the whole-number spelling `values.py` already derives for that pair. The
counterexample was in the tree before this gate: `${CORTEX_SUBAGENTS_MEM_BUDGET_GB:-8.0}` sits in
an environment block while `${CORTEX_SUBAGENTS_MEM_BUDGET_GB:-8}g` sits in two container limits,
because docker reads `8.0g` as a size and refuses it. A textual comparison would have called that
deliberate re-spelling a fault on the day it landed.
...
This gate sits here rather than inside `crosscheck.py`, which is registry-driven: every question
that scan asks starts from a hand-written entry naming a declaring site, and its documented subject
is a value some tree declares against the places restating it. This question has no registry and no
...
No compose file at all, a file that cannot be read or decoded, and a `$` form the reader was not
taught each fail rather than passing quietly, the same way both siblings do. A default this gate
cannot reduce fails only when its group already disagrees textually, since a value nobody re-spells
needs no reduction to be compared.

The success line states what the walk read: compose files, the variables they spend, and how many
of those were compared at all, that last being the collection the verdict is really over, since a
variable written once is never compared. The floor under it is `composefiles.py`'s; the deeper
counts get none, a tree whose variables are each written once being a legitimate one to find
nothing wrong with.
"""
...
def one_line_hint(spends: list[Spend]) -> str:
    ...
    Naming the same place twice is the only clue a reader gets about the commonest way to arrive
    ...
    measured on the tree: planting a note beside the model directory fails a group of five spends
