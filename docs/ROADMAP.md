...
- **The overlay's reminders-on-open surface landed 2026-07-14 ([ADR-0025 overlay
  ...
  tainted reminders** (the ADR-0025 deferral, now that a real badge exists to polish), and
  **"open the conversation this came from"**, which needs no wire change, since `session_id`
  already rides every row for exactly that.
