...
- **`tests/test_thinking_switch_live.py` measures whether a deployment honours `thinking=False`
  ...
  **recorded with the cells**, in one JSON sample per tier (`CORTEX_THINKING_OUT`,
  `CORTEX_THINKING_TAG`), beside the engine build, the model file and the context size the server
  reported of itself on `GET /props` (`build_info`, `model_path` and
  `default_generation_settings.n_ctx`, read once before anything is decoded, so a row quoted from
  the report names the build, the quant and the context it was measured at rather than the name
  the operator typed, ADR-0005 served-by and context-size addenda), and `just switch-tail`
  compares the prediction against the measurement and fails instead of publishing a run where the
  two disagree. The probe itself
