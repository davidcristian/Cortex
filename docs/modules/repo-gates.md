...
- `switchtail.py SAMPLE [SAMPLE ...]` is the fifth module here that gates nothing, and it is here
  ...
  **The reading is on the tail**, taken after the last of the ask the driver recorded sending,
  ...
  itself**: the sample's `build_info`, `model_path` and `n_ctx`, read once off `GET /props` and
  required by name like every other field, so a row copied into a record names the engine build,
  the file it was measured on and the context it was served at off the page rather than off the
  driver's notes, and a quant the lineup does not name is visible there (ADR-0005 served-by
  addendum). `n_ctx` is the one half of a row's placement that route reports; the GPU layer count
  is on no route llama-server offers and stays typed by hand beside it (ADR-0005 context-size
  addendum). Exit 0 printing the report; exit 1 printing it with a `refused:` line (a rendering it
