...
resolution is a walk down the desktop's Z-order in `body/crates/os_windows/src/focus.rs`, written
and clippy-linted for the Windows target from Linux, and **no walk in it has ever seen a real
desktop**. Every claim below is derived from Win32 documentation until this runs.

**How to drive it.** The brain half was added on 2026-08-10, so an ordinary question about a window
...
**Pass.** The picture is the **browser**, and the overlay is not in it and is not what was resolved.

**Fail.** The picture is the overlay, or is black, or is the taskbar. Black means the walk chose
a window that excludes itself from capture and the two guards that should have skipped it (this
...
**The reply and the receipt must agree, which is checkable from the assistant's side.** The reply
includes what the body resolved, picked by the same rule as the receipt, and the model is told
"screen capture of one window, cropped out of the WxH primary display". So the assistant describing
a window while the toast says "your screen", or the reverse, is a failure of that shared rule rather
than of either surface. Read the two together on the maximised-window case in particular, where both
must say screen.
