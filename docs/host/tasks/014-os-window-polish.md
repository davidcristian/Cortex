...
Only a real Win32 window shows whether transparency bleeds and whether click-through margins
behave.

## What it is, verbatim

Kept verbatim from [refinements/body-overlay.md](../../refinements/index.md#body-overlay), the entry that
moved here:
...
[windows-desktop.md](../index.md#windows-desktop) were still owed. Doing them first and this second is the
right order.
...
- 2026-07-19: Earlier the same day, before host work was extracted, the deferred-design backlog's
  paragraph on which areas tracked their host-side work named this entry as body and overlay's
  host-side validation item, while the same document elsewhere called it the only authoring entry
  that backlog held. Both readings stood in the old index; the pointer stub left at its origin and
  this task's own text both say authoring rather than validation.
- 2026-07-19: Moved out of the deferred-design backlog, where it had been an entry in the body and
  overlay area, and filed as host work with its text kept verbatim; the origin kept a dated pointer
  stub in place of the entry so the trail from an ADR through that backlog still resolves. Of
  everything that backlog held, this was the only entry that is authoring rather than deferred
  design anyone can pick up.
- 2026-07-19: The reason recorded for moving it rather than leaving it there under a tag is that the
  deferred-design backlog holds work anyone can pick up and its emptiness gates the README. That
  gate is dishonest if it waits on the user pressing a hotkey, and worse than dishonest here, since
  leaving this entry in that backlog would have meant it could not empty until the maintainer wrote
  Rust. The pointer stub left at the origin gave the same reason and named it the wrong contract for
  a backlog whose emptiness gates the README.
- 2026-07-19: Filed as a work session rather than a sitting, and as blocked on nothing. It was kept
  apart from the validation sittings because its bring-up and its failure mode are its own, and it
  was named the one piece of host work that is not urgent for correctness.
