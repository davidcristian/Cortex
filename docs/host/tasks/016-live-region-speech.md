...
The session was added on 2026-08-07, when the overlay's live region learned to report a list that
shrank and the tree stopped being able to answer the last question about it. It rides the same
bring-up as the Windows desktop sitting and can be folded into it if NVDA is already installed. It
is kept separate because it needs a reader and a speech viewer, and because it produces a transcript
rather than a pass.
...
[refinements/body-overlay.md](../../refinements/index.md#body-overlay) for the work. Then delete this
section, per the exit contract in [index.md](../index.md).

## Trail

- 2026-08-07: The host index sized this sitting at eight gestures in both of the places that gave it
  a size, its table of docs ("1 observation session (8 gestures)") and its roll call of items
  ("eight gestures read back through NVDA's speech viewer"). The gesture table above lists twelve
  rows, so the two sources disagree on the size of the session: eight by the index, twelve by this
  file's own table. Both readings are kept here rather than one of them picked.
- 2026-08-07: The host index's roll call filed this sitting under the heading "W, each with its own
  bring-up", while the same index's recommended order said it rides the same bring-up as the Windows
  desktop sitting and can be folded into that sitting if NVDA is already installed, which is what
  this file says as well, both above and in its bring-up section. Both readings are the index's own
  and both are kept.
