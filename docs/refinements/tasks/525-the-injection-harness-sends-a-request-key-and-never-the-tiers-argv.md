...
this repo starts has the pair on its argv and `PlacedAttempt` sends no key. A build on which the
request key and the server flag part ways, which the deprecation
[R-461](461-the-tiers-thinking-flag-is-deprecated.md) records as possible, would move the harness's
number without moving the tier's, and nothing would say so.
...
- 2026-09-04: closed. The premise held on the file as it stands after the image-budget and
  payload-size rows were added: `server_argv` took a `Budget` and nothing else, and `_post` sent
  ...
  `completion_body` puts in the request, and the text run happens once per entry in `SWITCHES`.
  Neither form is typed into the harness: `shipped_reasoning_off()` takes the pair off
  ...
  `test_switch_rows.py`, the CI-side check on the two rows. Measured over fifteen sessions on build
  `10680`: both gemma candidates drew identical cells under both switches every time, the pick at 0
  of 10 framed and 2 of 10 control; Qwen3.5-4B's `payload-splitting` cell fired once in four framed
  draws under each switch, which is the cell's own instability rather than the switch. The
  switch-row measurement of 2026-09-04 has the table, the argv-reached-the-engine check and the
  correction to the E2B row. Opened
