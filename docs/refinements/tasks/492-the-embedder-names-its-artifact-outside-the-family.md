...
back to the shipped nomic pick and load a different model with nothing reporting it, which is a worse failure
...
- 2026-08-30: landed as the rename, recorded in the [ADR-0029 addendum on a non-chat artifact
  ...
  family](../../adr/ADR-0029-vision-screen-capture.md),
  ...
  gone rather than left inert. **This entry's preferred close was rejected on the tree's own
  ...
  artifacts and exiting 0, so the exclusion was a general opening rather than one artifact's dispensation.
  **The shim this entry called the cheap shape was measured rather than assumed**: compose does
  ...
  `scripts/composedefaults.py` claimed and is corrected there, but that reader rejects a nested
