...
([ADR-0038 cheap-fold addendum](../../adr/ADR-0038-ranked-recall.md)).** The previous pass did not ship it
over its numbers; these are the numbers that let it ship on them. **Retention moved
...
chip on screen saying why, against 14.5 s to 224.5 s with nothing on screen. At the shipped floor the same
...
way, and the new default is pinned by a test that fails when it is flipped back. **What the
