...
- 2026-07-08: the deferred backoff and reconnect refinement landed as `RetryingTransport<T, S>`, a
  decorator over the unchanged `BrainTransport` port, with a lazy reconnecting constructor for the
  session-read path and `converse` forwarded unchanged so a failed turn stays terminal
  ([ADR-0024](../../adr/ADR-0024-transport-retry.md)). It left three refinements behind it: safe
  `converse` reconnect-before-first-event, a per-method or per-error-code policy, and a retry budget
  or circuit-breaker.
- 2026-07-13: jitter and the patient eager dial landed, adding the `Randomness` effect port, equal
  jitter with half the delay kept as a floor, a sanitized draw so a bad source cannot panic the
  `Duration` math, and `retry_with` composed around `converse.rs`'s eager dial.
- 2026-07-15: extracted from the ROADMAP's deferred-refinements section into this area doc, kept
  verbatim as the historical record of what the deferral became.
