...
gave the reader the form that entry asked for and left the half its own title named.
...
name. What catches it is `brain/packages/tools/tests/test_audit.py`, which asserts four whole
rendered lines, and the registry names that assertion so it cannot be deleted without a gate
reporting it (ADR-0009 declared-name addendum). One package's own suite, named by hand.
...
that group of names and fail on every one of them (ADR-0009 handed-message addendum).
...
`crosscheck.spend_fault` raises on a name spent where no MENTION renders the value under it, and here
the value is paid by the entry's own `Site`, so either a companion mention duplicates the site's
reading or the rule treats a site as paying the name it declares. The hole that leaves is an entry
...
each a failure. That is a change to `couplings.py`, which is today entirely read by `crosscheck.py`,
...
against it, so a call passing another word is a failure in the package that owns it. Neither is
