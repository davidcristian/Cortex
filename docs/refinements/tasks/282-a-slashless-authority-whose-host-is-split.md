...
- 2026-08-17: Landed. The host anchor reads a split host as a third host shape, and the grammar now
  ...
  lost, zero extended, zero identities changed, three added, all three this repo writing the attack
  spelling down. Every prose protection the eleventh addendum bought is intact, `https:no slashes
  here` included. Eleven behaviour tests, three mutation-proven breaks, a fourth tried and reported
  as a no-op rather than claimed. The streaming hold-back moved to `url_holdback.py` at the line
  cap in the same commit, per the split-as-you-go rule. Sibling entries: the tab inside a scheme
  word (285) is separate and untouched here; the host that mixes a dot and a gap (281) needs the
