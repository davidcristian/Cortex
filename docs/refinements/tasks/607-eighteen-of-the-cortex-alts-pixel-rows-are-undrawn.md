# Eighteen of the cortex alt's twenty-three pixel rows are undrawn
...
twenty-three alt rows. Five are drawn: the matrix and the laundering rate at the corpus frame and
the shipped budget, the picture-cost row at each budget, and the canary row. The other eighteen are
these:
...
sitting's two long rows, so the six frame and budget rows are about an hour together and each deep
row is over two hours.
