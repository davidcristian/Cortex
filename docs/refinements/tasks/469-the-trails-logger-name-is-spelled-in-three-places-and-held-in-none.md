...
benefit, and this repo reads such a constant where it exists rather than adding one for a gate. And the name is not one
...
- 2026-08-28: **landed**, as the [ADR-0038 named-logger
  ...
  **Both halves of this entry's account were stale.** The reopen trigger named a third document
  ...
  name, so the rename fails in the tree that writes the trail and goes unreported in the three documents
  ...
  reach every module reference in every module contract. `scripts/logcalls.py` gained the third
