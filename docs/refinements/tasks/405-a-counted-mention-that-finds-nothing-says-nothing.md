...
[R-403](403-a-needles-literal-reddens-the-wrong-entry.md), which taught an unfound needle to say
whose literal stopped matching and wired that into one of the two branches that can find nothing.
...
the reading was written for, and there are counted mentions over needles thick with a neighbour's
digits: `docs/runbooks/body-volume.md`'s `host.docker.internal:{value}` and
...
gone to zero in anger.

**What would close it.** Guard the count message on `found` and hand a zero to `needles.unfound`,
keeping the pinned count as a trailing clause so the reader still learns the mention was counted.
