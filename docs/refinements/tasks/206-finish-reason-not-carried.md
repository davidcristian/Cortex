...
- 2026-08-16: Landed as the port change it was priced as, ahead of a trigger that cannot fire, a
  ...
  off a live server, and files anything else under `UNKNOWN` rather than under silence. The loop
  ...
  never re-placed. `EchoInferenceBackend` answers too, truthfully, which is where the cadence's own
  refusal to answer stops applying. Proved end to end against the shipped CPU tier: a request capped
  at eight tokens came back `CAPPED` and the shipped attempt turned it into `ok=False`, where the
  same run used to return the cut title as an answer.
  ([ADR-0005 finish-reason addendum](../../adr/ADR-0005-llamacpp-engine.md).) It opened two
  narrower entries: the diagnosis half the recap fold still lacks
  ([R-277](277-a-cut-fold-reads-like-a-wandering-one.md)) and the one path where a capped run is
  still reported as a dead backend
