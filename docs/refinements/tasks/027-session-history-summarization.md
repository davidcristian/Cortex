...
Compressing old turns instead of dropping them changes
...
**Audited 2026-07-16 and kept deferred with the blocker sharpened ([ADR-0014
...
**Landed 2026-08-06 ([ADR-0038 summarizing-window addendum](../../adr/ADR-0038-ranked-recall.md)),
closing this area.**
...
- 2026-07-03: Deferred when session-history windowing landed. Compressing old turns instead of
  dropping them was left out as a lossy model pass that needs inference in turn assembly, weighed
  as an alternative in ADR-0014.
- 2026-07-16: Audited against the code together with the model-based reranker, which the index
  lists as one design problem with it, and kept deferred with the blocker sharpened. The async
  `select` widening both wait on is mechanically clean and contained, and the non-reentrant
  GPU-lease hazard is navigable by the title generator's sequential-drain discipline, so neither
  is the binding blocker.
- 2026-07-19: The audit's hardware blocker, that a model pass cannot be behavior-validated on the
  8 GB dev GPU because the cortex tier does not fit, was struck. That card holds the cortex and a
  model pass is judgeable there at 4K.
- 2026-07-19: The host page settled that same false clause per entry rather than recording it as
  merely stale, which had left the question with whoever picked the entry up. Its verdict was that
  this pass was never hardware-blocked at all, its real blockers being the shared `select` widening
  and the undecided cache question, and that what the host's hardware would still buy it is the
  same judgment at 16K with more than one slot, which it called a better answer rather than the
  only one.
- 2026-08-06: The two design questions closed under ADR-0038. `RecallPolicy.select` was widened
  once for its three waiting consumers, summarization's own question settled as cache rather than
  recompute, and the lease sequencing settled as the `drain_text` helper the title generator also
  moved onto.
- 2026-08-06: Landed the same day and shipped with `CORTEX_HISTORY_SUMMARY` default off, over
  the ADR-0038 summarizing, untrusted-recap, re-measured-behind-the-fence and cheap-fold addenda.
  `HistoryWindow.select` widened twice in the end, once for the `async` and the session id and
  once for a progress sink, which is the outcome the guidance existed to avoid; the reason it was
  still right to split them is that the sink had no consumer until a fold was slow enough to need
  narrating and cheap enough to ship.
- 2026-08-06: This model pass had stayed in the refinements backlog while listed among the work
  whose observation or judgment wanted the host's hardware, and it left that list the same day by
  being built, recorded there together with the reranker's pass as run and measured against the
  real cortex in Docker.
