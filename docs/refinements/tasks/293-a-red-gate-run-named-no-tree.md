...
tree that was green on both sides of it with nothing changed in between. The first was on the
commit that taught the backlog grammar to read a field's whole value when it wraps: four green runs
of the same gate on the same tree around it, one red. The second was on the commit that widened the
...
reds were single, both were bracketed by greens, and neither is explained.

**It did not reproduce.** After the second red, the gate was run twice more over the same tree
with the whole output kept, and both were green in 129 and 125 seconds. Two runs are not a rate,
and two greens are the outcome a genuinely intermittent failure gives most of the time, so this
narrows nothing. It does say the failure is not a standing property of that tree.
...
capture the whole of a failing hook run, never a tail of it. What a captured red looks like turned
up by accident during the two re-runs above: a third and fourth run went red in about a second
each, and their logs named the recipe, the tree and the reason on one line, because this file had
been written between runs and the index was stale. That red is explained and is not this one. It
...
for a given checkout an order is a pure function of its seed and an order-dependent red ought to
...
population the shuffle is honestly worth nothing for. And the four trees run in parallel under one
`just check`, so the load each sees differs run to run in a way no seed fixes, and anything timing
sensitive underneath moves with it. Neither of those was measured here. They are the first two
places to look, not findings.

**What would close it.** Nothing to build, which is why this waits. The next red has to be kept:
run the gate with its whole output captured, read the `FAILED` marker to get the tree, and take the
seed the failing suite printed in its own header. A red that names its tree and its seed is a bug
report somebody can act on; a red that names neither is this entry, and a third of those would say
only that the rate is not negligible.

## Trail

- 2026-08-17: written down after the second occurrence, on the pass that widened the anchor check
  to every document in the repo. Recorded rather than diagnosed: two reds, no tree named for
  either, a green re-run of the identical tree after each, and two further green runs that
  narrowed nothing.
