...
- 2026-08-21: Landed with the close of
  [342](342-the-audit-trail-cannot-name-the-turn.md). `SubagentTask` gained `session_id` and
  `turn_id`, written by `spawn_subagents` off the dispatch stamp and read back by the attempt, so a
  delegated tool call is audited under the chat and the turn that asked for it. It rides the record
  rather than a parameter because a subagent is a stateless function over the store, which is also
  why the ticker's fire (which stamps a chat and no turn) now reaches the trail honestly. Recorded
  in the ADR-0009 named-work addendum, with a pointer at the origin.
