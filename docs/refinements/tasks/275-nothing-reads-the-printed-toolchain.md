...
the measurement and then ran `rustcoverage.py`, which already checked those same two metrics plus
...
making `rustcoverage.py` the single result, so a coverage failure now names its metric and its
