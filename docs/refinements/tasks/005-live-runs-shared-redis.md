...
`limit=3` check on the same assumption and the trigger fell from fifty to three; the
...
a fortnight of a live run that would have blamed a correct adapter, and nothing reported it,
because these suites are run by hand. Reproduced on 2026-08-03 with sixteen real sessions present. **What it
...
fakeredis fixture already gives it. That also closed two siblings of the same defect with the
opposite symptom: the schedule and handoff live suites used to **skip** whenever the shared
database held a real record, passing while asserting nothing, and both skips are gone because
...
- 2026-08-03: Found and closed the same day, so the record is of what it was rather than of work
  ...
  `limit=50` window, lowered to three two days later by the pinning addendum without
