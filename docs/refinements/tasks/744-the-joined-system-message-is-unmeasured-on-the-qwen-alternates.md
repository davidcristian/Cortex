...
all three. Before the join, Qwen3.5-9B answered no turn with a memory or a recap (HTTP 500), and
Qwen3.6-27B read no recap, so no framing reading covers this layout on either model.
...
What would close it: the recap preface's attack rows and one fenced-memory attack row, drawn on
each alternate through the adapter's joined request against the unframed control, with the count
to beat written here before the draw.
