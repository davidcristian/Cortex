...
Something does that today, which the close of this entry found and this paragraph originally denied:
the tool audit attaches `arguments` verbatim, and `spawn_subagents` takes its `instruction` and
`context` from the model, so the unbounded field is on a trail this repo already writes rather than
in a future adapter. The rest of the reading held: every other field the tree attaches is an id, a
count, a flag, an endpoint or a short error detail, and the two per-line trails were designed
against exactly this risk. The defence that exists is about secrecy rather than size, a denylist of
