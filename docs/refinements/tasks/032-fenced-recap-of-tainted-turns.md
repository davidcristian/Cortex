...
the in-turn `Role.TOOL` message that carried the payload is turn-local and is discarded with
the turn,
the same finding the tainted-summarization decline made on the record path
([untrusted-content.md](../index.md#untrusted-content)). Nor is there a taint bit to key a rejection on: a
...
payload), each of the five fence sites mutation-proven to fail its own test. **The cost is
...
- 2026-08-06: Landed fenced at both ends. Settling it corrected the entry's own premise and found
  ...
  and the fix it needs is a persisted per-turn taint marker, which per-provenance eviction and a
...
- 2026-08-06: The fence's cost was left unmeasured, the live run having been made before the fence
  existed, so the usefulness question was opened beside the one-corpus entry that needs the same
  run.
