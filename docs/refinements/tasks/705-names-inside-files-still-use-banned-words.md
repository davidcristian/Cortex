...
[R-712](712-the-session-store-still-moves-the-hoisted-sets-first-key.md) closes; and the eleven
`seam` names the survey still finds, which are the `cortex_seam` package, `CORTEX_SEAM_TOKEN`, the
proto's `SeamError` message and eight test names that name that message. Strings are
