...
`restore_standing` fails in two places that name two different models: the eviction of the model
...
Closing it means `restore_standing` returning which model the attempt failed on, or nothing when it
...
- 2026-08-20: Fixed as ADR-0051 decision 7, with the paired question answered at the same time.
  `restore_standing` returns `str | None`: `None` is the permanent residency being back, and
  ...
  modules call. `restore_standing` has exactly one production caller, and the other two mentions
