...
`1..MAX_TOOL_DISPATCHES` at boot (a free tool and an unaffordable one both change what the
budget does without reporting it), and because a nested-dict env key replaces the whole mapping, the built-in prices
are merged *under* the user's so pricing one tool cannot unprice another. CI-gated
at 100% and mutation-proven (four guards, each reverted individually to a failing test). It also moved
`MAX_TOOL_DISPATCHES` into the new `tool_budget.py` beside the prices (one concern per module: that
