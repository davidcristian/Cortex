...
third placement target pending its feasibility pass, plus the surviving one of the two the
admission wall opened, a bounded admission wait, whose trigger is a turn observably stalled in
admission; its sibling, a read timeout on the subagent HTTP client, **landed 2026-08-09 and was
...
the same timeout design and `Clock` as the admission wait beside it; joined on
