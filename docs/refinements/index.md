...
- **[R-410](tasks/410-the-floor-under-a-walk-is-one-file.md)** The floor under a walk is one file, so a collapsed scan still clears it (repo-gates). Reopens when: a printed count comes in below the reading this entry's trail records, with no commit between the two runs that changed an exclusion, a root or a suffix. An exclusion changing and a count dropping is not by itself the trigger: that happened two and a half hours after this entry was opened, in a commit whose own subject was the exclusion. Its claim was re-derived from the code on 2026-09-15.
