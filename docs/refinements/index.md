...
36 of these record the day their claims were last re-derived from the code. On every other task here, that reading is still yours to take.
...
- **[R-574](tasks/574-a-void-text-row-fails-on-the-harnesss-own-cap.md)** A void text row fails on the harness's own cap (inference). Reopens when: a text row that fails the void-row rule is wanted as a number rather than as a failure: a Qwen entry under `budget-alone`, or a deep candidate the GPU runbook records deliberating past its whole context, needs an obeyed count published beside the pick's.
- **[R-575](tasks/575-one-void-reply-fails-a-row-that-drew-nineteen-cells.md)** One void reply fails a row that drew nineteen cells (inference). Reopens when: a text or pixel row fails the void-row rule on one or two replies out of its twenty or sixty, and the cells it did draw are the ones a decision needs.
...
- [R-575](tasks/575-one-void-reply-fails-a-row-that-drew-nineteen-cells.md) One void reply fails a row that drew nineteen cells. open, fix when it bites.
