...
**121 open, 299 closed, 420 in total.**
...
- **[R-357](tasks/357-a-replay-pass-has-no-cadence.md)** The record was replayed once, by a pass nothing schedules and nothing samples (cross-cutting).
- **[R-358](tasks/358-the-widest-value-was-never-a-real-line.md)** The widest value the tree attaches is a synthesis, and no real recall line was ever measured (memory).
- **[R-381](tasks/381-the-header-encoding-error-is-larger-than-recorded.md)** The grace margin's sizing cites a header error two orders of magnitude smaller than one measured (seam-transport).
- **[R-392](tasks/392-a-re-runs-second-deadline-outlasts-the-queue.md)** A re-run's second deadline outlasts the queue the first was ordered against (subagents).
- **[R-399](tasks/399-the-body-edge-is-two-sites-and-no-prose.md)** The body's own default edge is stated in prose that nothing reaches (repo-gates).
- **[R-414](tasks/414-the-still-spelled-reading-does-not-say-where.md)** The reading that says a file still spells a value does not say where it read one (repo-gates).
- **[R-417](tasks/417-the-swap-path-never-names-the-conversation.md)** The swap path never names the conversation, so a chat's evidence stops at the handoff (inference-model-manager).
...
- **[R-303](tasks/303-turn-stream-stall.md)** A turn stream that stalls has no bound at all (seam-transport). Reopens when: a turn observed stalling in real use, meaning the brain accepted the turn and then sent nothing for long enough that the user gave up on the thinking indicator, rather than failing it
...
- **[R-390](tasks/390-the-probes-mail-root-is-spelled-in-three-files.md)** The probe's mail root is spelled in three files and nothing can hold them together (repo-gates). Reopens when: the first edit that moves `/srv/mail`, or a fourth place spelling it, meaning somebody changing the probe's mail store and having to find the other two by reading.
...
- **[R-413](tasks/413-the-module-contracts-part-list-is-held-by-nobody.md)** The module contract names the registry's parts twice, and nothing holds either list (repo-gates). Reopens when: a part is added or renamed and the `scripts/` module contract keeps describing the set that existed before it, which is the first time either list being unheld costs a reader anything.
...
- **[R-419](tasks/419-the-git-call-inside-a-hook-is-written-three-times.md)** The environment strip that makes a git call inside a hook honest is written out three times (repo-gates). Reopens when: a fourth module needs to ask git something, or one of the three is edited by somebody who does not know why the environment is rebuilt, which is the first time the copies cost more than the lines they occupy.
- **[R-420](tasks/420-the-skipped-dirs-list-restates-what-git-ignores.md)** The hand-written skip list restates what git ignores, for every entry but one (repo-gates). Reopens when: a directory is added to `.gitignore` and a walk keeps reading it, or one is removed and a walk keeps skipping it, which is the first time the two lists disagreeing changes what a gate sees.
...
- [R-357](tasks/357-a-replay-pass-has-no-cadence.md) The record was replayed once, by a pass nothing schedules and nothing samples. open, actionable.
...
- [R-417](tasks/417-the-swap-path-never-names-the-conversation.md) The swap path never names the conversation, so a chat's evidence stops at the handoff. open, actionable.
...
- [R-358](tasks/358-the-widest-value-was-never-a-real-line.md) The widest value the tree attaches is a synthesis, and no real recall line was ever measured. open, actionable.
...
- [R-390](tasks/390-the-probes-mail-root-is-spelled-in-three-files.md) The probe's mail root is spelled in three files and nothing can hold them together. open, fix when it bites.
...
- [R-399](tasks/399-the-body-edge-is-two-sites-and-no-prose.md) The body's own default edge is stated in prose that nothing reaches. open, actionable.
...
- [R-413](tasks/413-the-module-contracts-part-list-is-held-by-nobody.md) The module contract names the registry's parts twice, and nothing holds either list. open, fix when it bites.
- [R-414](tasks/414-the-still-spelled-reading-does-not-say-where.md) The reading that says a file still spells a value does not say where it read one. open, actionable.
...
- [R-419](tasks/419-the-git-call-inside-a-hook-is-written-three-times.md) The environment strip that makes a git call inside a hook honest is written out three times. open, fix when it bites.
- [R-420](tasks/420-the-skipped-dirs-list-restates-what-git-ignores.md) The hand-written skip list restates what git ignores, for every entry but one. open, fix when it bites.
...
- [R-303](tasks/303-turn-stream-stall.md) A turn stream that stalls has no bound at all. open, fix when it bites.
...
- [R-381](tasks/381-the-header-encoding-error-is-larger-than-recorded.md) The grace margin's sizing cites a header error two orders of magnitude smaller than one measured. open, actionable.
...
- [R-392](tasks/392-a-re-runs-second-deadline-outlasts-the-queue.md) A re-run's second deadline outlasts the queue the first was ordered against. open, actionable.
