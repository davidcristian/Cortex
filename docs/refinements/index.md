...
What replaces it is narrower and is the same exemption read to its edge: a paste is exempt from the
width and from nothing else, so a bare `--` separator or a hash that resolves is still refused
inside a fence, whose trigger is the first commit whose paste carries either
