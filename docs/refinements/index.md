...
**126 open, 323 closed, 449 in total.**
...
- **[R-448](tasks/448-the-module-listing-is-held-whole-and-not-in-halves.md)** The gate tree's module listing is held whole, so neither of its halves is (repo-gates). Reopens when: a module gains or loses a CLI and is left in the wrong half of the sentence, or the count in front of the second half stops matching it, which is when a reader is told a module is something it is not.
- **[R-449](tasks/449-the-repo-map-names-every-gate-module-unheld.md)** The repo map names every gate module in a block no reader here can see (repo-gates). Reopens when: a module is added to `scripts/` and the repo map keeps describing the tree that existed before it, which is the same drift the module contract's own listing was just held against, one document over.
...
- [R-448](tasks/448-the-module-listing-is-held-whole-and-not-in-halves.md) The gate tree's module listing is held whole, so neither of its halves is. open, fix when it bites.
- [R-449](tasks/449-the-repo-map-names-every-gate-module-unheld.md) The repo map names every gate module in a block no reader here can see. open, fix when it bites.
