...
**95 open, 234 closed, 329 in total.**
...
- **[R-315](tasks/315-subagent-cpu-budget-and-its-siblings.md)** The subagent CPU budget, and the sibling asks beside it (repo-gates).
- **[R-322](tasks/322-brain-reads-the-remaining-time.md)** The brain is told how long it has and does nothing with it (seam-transport).
...
- **[R-329](tasks/329-a-failure-with-two-candidate-subjects.md)** A failure that wraps two calls names neither model (inference-model-manager).
...
- **[R-316](tasks/316-a-rank-fallback-cannot-name-its-turn.md)** A rank fallback cannot name its turn (memory).
...
- **[R-307](tasks/307-shortcut-reference-link-in-a-heading.md)** A shortcut reference link in a heading is the one shape the refusal cannot see (repo-gates). Reopens when: the first link reference definition written anywhere in this repo, a line whose whole content is a bracketed label, a colon and a target. Until one exists, the shape below renders as literal brackets and both readings agree.
...
- **[R-313](tasks/313-a-relay-can-be-required-and-empty.md)** A required toolchain relay can still arrive empty (repo-gates). Reopens when: a coverage verdict prints `measured by` with nothing after it, or the recipe stops probing the compiler on its own standing line before it measures.
...
- **[R-324](tasks/324-a-rendered-field-has-no-bound.md)** A rendered field has no bound on its length (cross-cutting). Reopens when: a line in `docker compose logs brain` that scrolls past a screen, or any adapter that attaches a value it did not build itself
- **[R-325](tasks/325-a-raised-message-is-also-a-logged-one.md)** A message that is raised and logged keeps its values in prose (cross-cutting). Reopens when: a seventh site of this shape arriving, or one where the prose value and the field beside it disagree
...
- [R-324](tasks/324-a-rendered-field-has-no-bound.md) A rendered field has no bound on its length. open, fix when it bites.
- [R-325](tasks/325-a-raised-message-is-also-a-logged-one.md) A message that is raised and logged keeps its values in prose. open, fix when it bites.
...
- [R-329](tasks/329-a-failure-with-two-candidate-subjects.md) A failure that wraps two calls names neither model. open, actionable.
...
- [R-316](tasks/316-a-rank-fallback-cannot-name-its-turn.md) A rank fallback cannot name its turn. open, a seam or port change comes first.
...
- [R-307](tasks/307-shortcut-reference-link-in-a-heading.md) A shortcut reference link in a heading is the one shape the refusal cannot see. open, fix when it bites.
- [R-308](tasks/308-crosscheck-cannot-tie-a-decimal.md) The constant scan cannot tie a decimal. landed 2026-08-19.
- [R-313](tasks/313-a-relay-can-be-required-and-empty.md) A required toolchain relay can still arrive empty. open, fix when it bites.
- [R-314](tasks/314-decimal-form-refusals.md) What the decimal value form still refuses. open, dead until a consumer.
- [R-315](tasks/315-subagent-cpu-budget-and-its-siblings.md) The subagent CPU budget, and the sibling asks beside it. open, actionable.
...
- [R-322](tasks/322-brain-reads-the-remaining-time.md) The brain is told how long it has and does nothing with it. open, actionable.
