...
**161 open, 503 closed, 664 in total.**
...
- **[R-665](tasks/665-a-settled-handoff-does-not-say-it-was-cut.md)** A settled handoff record does not say whether the answer was cut (inference-model-manager). Reopens when: the first request to count cut handoffs, or to tell a clean handoff from a cut one after the log has rolled. Its claim was re-derived from the code on 2026-09-14.
- **[R-666](tasks/666-the-inward-skip-check-asks-only-the-three-repo-wide-walks.md)** The inward skip check asks only the three repo-wide walks (repo-gates). Reopens when: a directory git ignores appears under one of the three subtrees a scoped reader descends, `brain/packages/` for `logcalls.py`, the package suite `assertedlines.py` is pointed at, or `docs/runbooks/` for `samplecheck.py`, and holds a file that reader would open. Its claim was re-derived from the code on 2026-09-14.
- **[R-667](tasks/667-a-dateless-row-is-passed-over-by-the-commit-anchor.md)** A dateless row is passed over by the commit anchor (repo-gates). Reopens when: a pass writes a ledger row whose "Drawn from" cell holds no commit this clone resolves while an earlier row's does, so the standing line counts from a pass that is not the last one and reports more unsampled work than there is. Its claim was re-derived from the code on 2026-09-15.
- **[R-668](tasks/668-a-rewritten-history-unreproduces-a-recorded-draw.md)** A rewritten history unreproduces a recorded draw (repo-gates). Reopens when: somebody tries to reproduce a ledger row's sample from its seed and its tip, and the five bodies the draw returns are not the ones the row says were replayed. Its claim was re-derived from the code on 2026-09-15.
...
- [R-666](tasks/666-the-inward-skip-check-asks-only-the-three-repo-wide-walks.md) The inward skip check asks only the three repo-wide walks. open, fix when it bites.
...
- [R-668](tasks/668-a-rewritten-history-unreproduces-a-recorded-draw.md) A rewritten history unreproduces a recorded draw. open, fix when it bites.
