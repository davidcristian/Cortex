...
measured and whose fix is a sweep over every evicted tier, triggered by the first deployment
...
the cause since a boot has had none. One takes its place, **the deep model's own clearing**, which
is deliberately still fatal to that verdict and rightly so for an unreachable host and for a deep
model that really will not stop, but wrong for a deployment that turned escalation on without
naming `CORTEX_MODEL_FILE_BRAIN`, whose tier the daemon 404s for ever; the port cannot tell those
apart today, so the fix is a narrower failure on it and the trigger is the first boot observed
amber over a serving cortex
