...
**176 open, 429 closed, 605 in total.**
...
- **[R-378](tasks/378-the-barrel-rule-omits-two-root-internals.md)** The barrel rule and the module contract describe two different surfaces (repo-gates). Reopens when: something outside `cortex_orchestrator` imports one of the composition root's own types. That is countable by searching the tree for an import of `cortex_orchestrator.stores`, `cortex_orchestrator.engines`, `cortex_orchestrator.preference_servicer` or `cortex_orchestrator.session_servicer` from a file outside that package: the trigger fires when one is found.
