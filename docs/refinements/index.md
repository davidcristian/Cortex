...
([body-overlay.md](body-overlay.md)); the second of the two bounds the panel's section budget
leaves behind it, joined on 2026-08-03 when the budget landed and halved on 2026-08-08 when the
first of them landed (a section's own border, padding and air sat under no cap at all, so two open
sections cost 40px the budget could not reach and the hint strip stood 34px outside a 640x240
viewport; the share now bounds the section's wrapper, which has no frame of its own to floor it, and
the shortest viewport everything fits in went 286px to 220px, the entry's own boundary having been
284px rather than the "roughly 260px" it named): the room a closing section hands back arrives in a
single frame, because the
