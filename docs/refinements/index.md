...
**97 open, 206 closed, 303 in total.**
...
- **[R-302](tasks/302-brain-learns-the-deadline.md)** The brain is never told the deadline the body is holding it to (seam-transport).
...
- **[R-290](tasks/290-the-export-names-its-tool-not-its-compiler.md)** The coverage export names its tool and never its compiler (repo-gates). Reopens when: a coverage failure where the relayed `rustc` line is not enough to settle whether the compiler moved, or cargo-llvm-cov recording the compiler in its export, which would make the check free.
...
- **[R-292](tasks/292-slug-rule-approximates-a-renderer.md)** One regex stands in for a renderer's slugger (repo-gates). Reopens when: the first heading written in one of the five shapes below, or any report from this gate against a heading that a renderer resolves fine, either of which turns a measured absence into a live disagreement.
...
- **[R-297](tasks/297-cut-tool-call-fails-the-cortex-turn.md)** A cut tool call fails the cortex turn as an inference error (inference-model-manager). Reopens when: the first cortex turn observed ending in an inference-failed seam error whose text quotes a tool call's unterminated arguments.
- **[R-298](tasks/298-served-ids-are-opt-in-everywhere.md)** A test backend serves every model id unless it is told otherwise (repo-gates). Reopens when: the first turn, fold, delegation or handoff that reaches a real deployment with a model id its wiring never had, having passed every test that covers that path.
...
- [R-297](tasks/297-cut-tool-call-fails-the-cortex-turn.md) A cut tool call fails the cortex turn as an inference error. open, fix when it bites.
...
- [R-290](tasks/290-the-export-names-its-tool-not-its-compiler.md) The coverage export names its tool and never its compiler. open, fix when it bites.
...
- [R-292](tasks/292-slug-rule-approximates-a-renderer.md) One regex stands in for a renderer's slugger. open, fix when it bites.
- [R-293](tasks/293-a-red-gate-run-named-no-tree.md) A red gate run that named no tree. open, fix when it bites.
- [R-298](tasks/298-served-ids-are-opt-in-everywhere.md) A test backend serves every model id unless it is told otherwise. open, fix when it bites.
...
- [R-302](tasks/302-brain-learns-the-deadline.md) The brain is never told the deadline the body is holding it to. open, actionable.
