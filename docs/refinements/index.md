...
**96 open, 197 closed, 293 in total.**
...
- **[R-285](tasks/285-a-tab-inside-a-scheme-word.md)** A tab inside a scheme word (untrusted-content). Reopens when: a reply or an untrusted result spelling a scheme or its separator with a tab inside it, which is the one position of the three where the shipped grammar anchors nothing
...
- **[R-289](tasks/289-reasoning-budget-is-all-or-nothing.md)** The reasoning budget is all or nothing (inference-model-manager). Reopens when: a build of llama.cpp, or a request field it accepts, that bounds `reasoning_content` by a token count rather than switching it off.
...
- [R-289](tasks/289-reasoning-budget-is-all-or-nothing.md) The reasoning budget is all or nothing. open, fix when it bites.
