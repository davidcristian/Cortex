...
**153 open, 524 closed, 677 in total.**
...
- **[R-681](tasks/681-the-evict-list-accepts-the-deep-tier-it-makes-room-for.md)** The evict list accepts the deep tier it makes room for (resource-governance). Its claim was re-derived from the code on 2026-09-17.
...
- **[R-023](tasks/023-converse-reconnect-first-event.md)** Safe `converse` reconnect before the first event (seam-transport). Reopens when: a deployment that sets `CORTEX_ESCALATION`, which is the switch that builds a swap scope at all (`brain/packages/orchestrator/src/cortex_orchestrator/swap_builders.py:103` returns `None` without it), together with turns costly enough that a silent re-run beats paying for dedup. Recheck with `grep -rn CORTEX_ESCALATION docker/`: one hit, inside a comment, means nothing swaps and this has not fired. Its claim was re-derived from the code on 2026-09-17.
- **[R-024](tasks/024-disconnect-mid-handoff-teardown.md)** A disconnect mid handoff blocks stream teardown (seam-transport). Reopens when: a client event that does not need the cortex, observed waiting behind a `Cancel`'s restore on a stack running handoffs. That observation is off-tree: no stack here runs a handoff, and `grep -rn CORTEX_ESCALATION docker/` reporting its one comment hit (`docker/docker-compose.gpu.yml:25`) says it still cannot be made. Its claim was re-derived from the code on 2026-09-17.
...
- [R-681](tasks/681-the-evict-list-accepts-the-deep-tier-it-makes-room-for.md) The evict list accepts the deep tier it makes room for. open, actionable.
