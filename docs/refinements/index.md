...
**154 open, 524 closed, 678 in total.**
...
- **[R-682](tasks/682-the-recall-policies-restate-what-they-share.md)** The recall policies restate what they share, one policy at a time (memory). Its claim was re-derived from the code on 2026-09-17.
...
- **[R-018](tasks/018-ports-without-contract-suite.md)** Ports without a shared contract suite (repo-gates). Reopens when: a Rust port or `EmailSender` gaining a shared check list, which answers the design question below for its language; or a Rust test passing over a fake while the adapter it stands in for fails the same expectation, readable in CI for `BrainTransport`, `Sleeper` and `Randomness` and only on the Windows host for the four OS ports. Its claim was re-derived from the code on 2026-09-17.
...
- [R-682](tasks/682-the-recall-policies-restate-what-they-share.md) The recall policies restate what they share, one policy at a time. open, actionable.
