...
**173 open, 447 closed, 620 in total.**
...
76 of these record the day their claims were last re-derived from the code. On every other task here, that reading is still yours to take.
...
- **[R-512](tasks/512-no-committed-probe-splits-the-reasoning-off-pair.md)** No committed probe sends the request's own trace budget, or pairs two arms at a seed (inference). Its claim was re-derived from the code on 2026-09-09.
...
- **[R-575](tasks/575-one-void-reply-fails-a-row-that-drew-nineteen-cells.md)** One void reply fails a row that drew nineteen cells (inference). Its claim was re-derived from the code on 2026-09-09.
...
- **[R-608](tasks/608-the-cost-rows-assertions-are-the-picks-saturation-and-the-alt-fails-both.md)** The cost row's assertions are the pick's saturation and the alt fails both (inference). Its claim was re-derived from the code on 2026-09-09.
...
- [R-575](tasks/575-one-void-reply-fails-a-row-that-drew-nineteen-cells.md) One void reply fails a row that drew nineteen cells. open, actionable.
...
- [R-608](tasks/608-the-cost-rows-assertions-are-the-picks-saturation-and-the-alt-fails-both.md) The cost row's assertions are the pick's saturation and the alt fails both. open, actionable.
- [R-611](tasks/611-nothing-reads-the-build-the-engine-names-on-every-response.md) Nothing reads the build the engine names on every response. open, actionable.
...
- [R-615](tasks/615-nothing-reads-a-subagent-result-back-from-the-store.md) Nothing reads a subagent result back from the store. open, actionable.
...
- [R-604](tasks/604-the-advisory-probes-arms-part-by-fourteen-draws.md) The dialog probe's two arms part by fourteen draws, in the direction the defence is not for. open, actionable.
- [R-605](tasks/605-the-bare-probe-rests-on-one-sitting.md) The unstyled probe carries half the square's answer and has one sitting behind it. open, actionable.
- [R-606](tasks/606-the-alt-reports-the-dialogs-rule-in-grammar-the-tail-reading-sorts-as-applied.md) The alt reports the dialog's rule in grammar the tail reading sorts as applied. open, actionable.
