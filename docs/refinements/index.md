...
**158 open, 509 closed, 667 in total.**

## What remains

158 of these record the day their claims were last re-derived from the code. On every other task here, that reading is still yours to take.
...
- **[R-671](tasks/671-the-widest-line-check-names-its-sinks-by-hand.md)** The widest-line check names its sinks by hand (cross-cutting). Reopens when: a third sink writing a log line whose fields a caller or a model can fill. The sinks that exist are read off the composition root, `grep -rn "Logging.*Sink" brain/packages/*/src`, which names both the class and the builder that wires it; a name in that answer with no case in `brain/packages/orchestrator/tests/test_widest_line.py` is the trigger fired. Its claim was re-derived from the code on 2026-09-15.
...
- [R-671](tasks/671-the-widest-line-check-names-its-sinks-by-hand.md) The widest-line check names its sinks by hand. open, fix when it bites.
