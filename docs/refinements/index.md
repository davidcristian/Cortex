...
| Doc | Area | Open |
| --- | --- | --- |
| [repo-gates.md](repo-gates.md) | Line cap (the core barrel came off it 2026-08-06, split into area sub-barrels under `cortex_core._surface` with every call site unmoved, ADR-0026), dashcheck, coverage config (ADR-0026), gate coverage of the ungated Rust trees and of the overlay's TypeScript (ADR-0011), the stylesheet still outside the cap, test-runner mechanics (ADR-0002), whose live runs now each own their store after the pgvector one took the `cortex_contract` database 2026-08-06 and the Redis ones took a logical database 2026-08-03, the five couplings the widened constant scan still cannot hold, which is what is left after four of its predecessor's five kinds closed 2026-08-08 behind an ordering comparator and a mention form for a far side that spends a value without declaring it (ADR-0029). This cell called them three and was wrong the day it was written, the fourth having been folded into that entry hours later when the kinded gateway errors needed both languages to spell one status code alike. They are four again on 2026-08-09 by exchange rather than by standing still: the roll's duration closed onto `--roll: 300ms` on `:root`, which the two rules that move WITH a roll spend and the scan holds against `MORPH_ROLL_MS`, and its own account of the tree was seven spellings of `0.3s` rather than the "some thirty inline sites" it claimed, six of them declarations and only two of those six the roll; in its place opened the spend side of a value-declaring property, since a mention renders a value and so reaches the declaration on `:root` but never the `var(--roll)` that pays it, which `--ease` has been living with since the registry widened. The harness that measured a whole recalling turn through the seam and stayed in a scratchpad was added 2026-08-08, sharpened hours later by the fold-under-load run to the two halves only a driver going over gRPC meets, and closed 2026-08-09 on the other half of its own trigger, the default reproduced rather than cited. Both halves have one answer and it is a division of labour: an arm is a container configuration, so the restarts live in `just turn-cost`, which puts the arms in separate processes, which is why each block writes a sample file and the interval is computed afterwards by `scripts/contrast.py`, the first module in that tree that gates nothing, leaving the block driver asserting only invariants. The A/B/A control was never a second question, being the recipe running its outer blocks in one configuration. Two of the entry's claims failed against the tree: seeding was said to be settled by the fold run, whose corpus is conversation history through `RedisSessionStore` while this one is 41 memory notes through `PgVectorMemoryStore` behind the embedder, so only the discipline carried; and the harness could not have been written when it was filed, no `CORTEX_MEMORY_*` knob having reached the dockerized brain at all, which the memory runbook had been documenting past ever since the override was written. The compose bind defaults that land in the repo tree closed 2026-08-08 ahead of their trigger as a fourth cross-tree scan, and the entry's own sketch of the rule did not survive the tree: three of the nine binds point at files the repo ships, so "fail when a default is not gitignored" would have failed on inputs, and the rule that shipped is outside, or tracked, or ignored. A mention counting nothing closed 2026-08-09 on its trigger as an optional exact occurrence count, exact rather than a floor because a floor cannot notice the far side has grown past it, and opt in so that the two far sides whose spellings are one set are pinned (`Message.tsx` at 2, the two morph share caps at 2 under a narrower template) while the three `[data-morphing` rules stay a presence check, being the sum of two unrelated features. That close leaves this count where it found it rather than lowering it: the entry was appended on 2026-08-08 and never added here or to the area header, so both read 6 while seven were open, and six is true from today. Six is still true on 2026-08-09, read entry by entry rather than carried forward: standing test-order randomization, the shell's `cargo clippy` in CI, the stylesheet outside the line cap, the couplings entry above, the turn-cost harness, and, from later that day, a paste being exempt from the wrap and from nothing else, which replaced the wrap gate's three unshipped exceptions when those closed ahead of their trigger, no commit in 433 having ever needed a command or a block in its body: the wrap now walks the message carrying a fence toggle, steps over a fenced line and a `$` prompted paste, and reports a fence left open rather than letting one exempt every line after it, while the `BREAKING CHANGE:` footer was decided rather than exempted, wrapping like the prose it is, and that entry's sharpest claim, that the gate could refuse a message the commit rules mandate, did not survive being measured, a wrapped footer having always passed. The roll close moves nothing here, one sub-coupling out of that entry and one in. **Five later that same day**, when the paste entry closed ahead of its trigger, which had not fired: 437 commits hold 0 fenced lines and 1 prompt-marked one, and that one carries neither a bare `--` nor a hex token, so what moved it is that the facility is now in use and the commands this repo would paste are its own gate invocations, three of which spell the separator. The answer is per rule rather than blanket: a paste is exempt from the wrap and from the dash ban, since both are about the text as typed and neither has a remedy inside verbatim words, and it is not exempt from the volatile-reference ban or the resolving-hash check, since those are about the message still reading correctly after what it points at moves, which does not care who typed the pointer and whose remedy a paste keeps. The entry's own narrower reading, exempting only cargo's `--` and a fenced hash, was refused for being a rule about character sets rather than kinds. Five is read entry by entry: standing test-order randomization, the shell's `cargo clippy` in CI, the stylesheet outside the line cap, the couplings entry above, and the turn-cost harness. **Four later that same day**, when the turn-cost harness closed on the second reading of its trigger, and this one lowers the number rather than exchanging it: what the close had to build on the way, the pass-through block that finally lets a `CORTEX_MEMORY_*` knob reach the dockerized brain, is a defect fixed rather than an entry opened, and the block driver's own limits are named in the addendum instead of here. Four is read entry by entry, the same four the reading above named minus the harness: standing test-order randomization, the shell's `cargo clippy` in CI, the stylesheet outside the line cap, and the couplings entry above. **Still four on 2026-08-10**, when the couplings entry grew a fifth coupling and no fifth entry: the capture target's proto enum against the JSON-schema strings the model picks between, the first coupling here whose both sides are generated, so the scan has nothing to parse in either. A coupling arriving inside that entry is recorded there and changes no number here, the same treatment the gRPC status codes got; the half of it a gate can hold is held without one, the schema list being derived from the domain enum rather than restated. **Still four later that same day**, read entry by entry rather than carried, when the two triggers here that a command can settle were run rather than read, which is what the sweep the day before could not do: five seeds of `pytest-randomly` supplied for the run only over the whole brain workspace (2306 tests) and five more over `scripts/` (400) are all green with the collected order provably different per seed, and the Tauri shell's `cargo clippy --all-targets -- -D warnings` exits 0 over the 978 lines in 12 files it now holds, both proven able to fail before being trusted. Neither fired and nothing opened or closed, so the four are standing test-order randomization, the shell's clippy in CI, the stylesheet outside the line cap, and the couplings entry above. Both run entries had their own text corrected by the running: the brain workspace is 2306 tests where the randomization entry records 1642, and the shell is 978 lines where its entry records 881 and needs no `pkg-config` shim on this host, only 47 `-dev` packages of `.pc` metadata unpacked without sudo, which is the CI provisioning cost the decline rests on, measured rather than restated. **Five later that same day**, by arrival rather than exchange, when the `MemoryStore` contract fix of the day before was swept out to every port in both languages and found the identical defect on exactly one more, `SessionStore`, whose fourteen shared checks were read only by the integration-marked live-Redis run while the CI driver restated all fourteen by hand, so a fifteenth would have reached CI only if somebody wrote it twice. That one closed inside the sweep, parametrized over its own tuple like the other four stores beside it and proven able to fail, a poisoned fifteenth check reddening both implementations where the restated driver had answered `66 passed` over it. What opened is the half a sweep can only measure: four Python ports have a fake, a real adapter and no shared list (`Embedder`, `ToolRegistry`, `BodyGateway`, `Confirmer`), `InferenceBackend` has one for its decode cadence and nothing shared for the rest of streaming, no Rust port has one at all (where the fakes themselves are restated, `FakeAudio`, `FakeNotify`, `FakeScreen` and `FakeBrain` each written twice in two crates with independent expectations), and the overlay's `BrainBridge` has three implementations of which one is tested, the other two named in `vite.config.ts`'s coverage `exclude`. The full port inventory, including the ports whose fake and adapter legitimately cannot share checks and why (the write-only sinks, `Clock`, `Sleeper`, and `ZoneResolver`, whose two implementations are deliberately not interchangeable), is the ADR-0001 addendum on decision 2 rather than the entry, so the next sweep re-reads rather than re-derives. Five is read entry by entry: standing test-order randomization, the shell's clippy in CI, the stylesheet outside the line cap, the couplings entry, and the ports without a contract suite. **Still five on 2026-08-11**, when the sharpest row inside that last entry closed without the entry closing: the overlay's `BrainBridge` got the first shared check list outside Python, thirteen named checks in `body/app/src/bridge/bridgeContract.ts` driven over `FakeBridge` and `DemoBridge` by one parametrized driver, and `demoBridge.ts` and its script came out of the overlay's coverage `exclude` behind it, leaving `main.tsx` and the IPC-crossing `tauriBridge.ts` there with their reasons beside them. The list paid before either implementation was changed to suit it, reddening on three disagreements against the port's own description: a fake that ignored its listing limit, a fake whose preference write the served record never carried while its three catalog writes reflected theirs, and a demo bridge that read a zero limit as "at most none" where the port documents the brain's own default, with a fourth out of the turn-handle check rather than out of the two arms disagreeing, the demo announcing a capture inside the `converse` call where the real bridge's channel can only deliver after it. What stays open there is the four Python ports, the streaming half of `InferenceBackend`, and every Rust row. **Still five later that same day**, when the first of those four Python ports gained its list and the entry stayed open: `Embedder`, four checks in `brain/packages/embedding/tests/embedder_contract.py` driven over `HashEmbedder` and over `LlamaCppEmbedder` on a `MockTransport` whose stand-in server answers JSON integers, so the check on float elements is about the adapter's coercion rather than about the transport. It found no behavioural disagreement, which is the honest outcome for a port one method wide and is recorded rather than left as a silence, and it found a fake that could not raise the one error the port names, so nothing in the core could exercise a remember or a recall against a dead embedding server; `HashEmbedder` gained the scripted `fail_with` that `InMemoryBodyGateway` has carried since it was written. Proven able to fail once per arm and once per side of that knob, each break restored. `ToolRegistry`, `BodyGateway` and `Confirmer` stay open. **Still five later that same day** for the second of the four, `ToolRegistry`, which is the one that paid: six checks driven over three implementations, the core's fake and both MCP registries, since the translating one and the reconnecting one production wires are not the same implementation of every promise. The fake could express neither the port's central case (a tool that ran and reported an error, where every core test of a failing tool went through the other branch, a handler raising, which the dispatcher labels differently) nor its world (a tool set copied at construction, and no way to be unreachable), and it gained all three. One divergence was decided against the port rather than against an implementation: it promised `ToolNotFoundError` for an unknown name, which only a registry knowing its whole set can keep, so the description now states the safety half both owe, that a name an implementation does not serve never comes back as a success. Proven able to fail four times, each on the arms that can carry the defect, including a listing cache that reddens one MCP arm and not the other. **Still five later that same day** for the third, `BodyGateway`, whose ten checks run over the fake and over the real adapter talking to a `BodyService` on loopback, and whose finding runs the dangerous way: the fake handed back a capture the adapter would have refused, since a non-zero `max_edge` or `max_bytes` is a bound on the reply and only the adapter re-verified it, so a core test could watch a turn accept a picture production would have thrown away. That rule is domain logic rather than wire translation and moved into the core, where both implementations now call it. Proven able to fail four times, twice per side. **Still five later that same day** for the fourth and last of them, `Confirmer`, whose five checks run over the core's fake and over `SeamConfirmer` with a scripted overlay wired into its `emit`, reading the card off the control path and answering through `resolve` as the Converse stream does. The fake's answer was fixed at construction and a person is not a constant, so it could not be asked about two calls in one run; no behavioural disagreement came out of the five, which for a port whose whole contract is that only an explicit yes is `True` is the answer worth having. Proven able to fail three times and once deliberately not, a `resolve` that answers whichever ask is pending leaving every shared check green because only one ask is ever outstanding through the port, which is the division of labour rather than a hole. The four Python ports the sweep named are done and the entry is still open on `InferenceBackend`'s streaming half and every Rust row. **Still five later that same day**, when the couplings entry lost one of its five couplings and stayed open: the membership closed as `Relation.MEMBER` plus a `frozenset` value form, which ties the body's `CAPTURE_MIME` inside the brain's `ALLOWED_MIME_TYPES` and takes the registry to sixteen entries, and a coupling leaving that entry moves no count here for the same reason an arriving one does not. The line cap is what the close actually cost, the scan and the registry both being within twenty lines of 300, so the value forms and the relation comparators are `scripts/values.py` now and the overlay's half of the registry is `scripts/overlaycouplings.py`, with `crosscheck.CONSTANTS` reading the two halves as one and nothing in the scan asking which half an entry is in. Five is read entry by entry: standing test-order randomization, the shell's clippy in CI, the stylesheet outside the line cap, the couplings entry, and the ports without a contract suite. **Still five later that same day** for the second of that entry's couplings, the custom property whose spend a rendered value cannot reach, closed as `Mention.name` plus a `{name}` placeholder so a template can pin a rendered NAME: the pair is two mentions of one entry, the declaration on `:root` and the `var()` that pays it, and the registry refuses a name pinned as a spend that no mention renders a value under. `var(--roll)` is counted at 2, being the set the entry's own reason names, and `var(--ease)` is a presence check because 52 transitions ride that curve, which is the same division the three `[data-morphing` rules got. Proved able to fail three times where the previous commit's scan is green on two of them. What it leaves is narrower than what it closed and is recorded in the entry: a mistyped spend of a presence-checked name is still invisible, and the honest close of that is a stylesheet-wide check that every `var()` names a property something declares, which is a different scan over a different input | 5 |
...
| [tools-mcp.md](tools-mcp.md) | Dispatch budget/cost/salience, spawn batch cap, MCP registries (ADR-0009/0010), one fewer since 2026-08-08 when the sidecar session pool closed **declined on measurement**: the per-call open its entry called acceptable without a number is 17.8 ms on the transport this repo's own sidecars serve, and the 565 ms the reference filesystem sidecar charged for the same open was not the handshake at all but that sidecar spawning an `npx` process per JSON-RPC request and never reaping it, 1452 of them holding 20.5 GiB after a few hundred calls, so the fix was the compose command rather than a pool and the pre-token walk went 1156 ms to 146 ms with no brain code changed. Its entry's "behind the same `ToolRegistry` port" was false in the other direction too, a pooled session needing a scope that all seven combinators would forward | 5 |
| [untrusted-content.md](untrusted-content.md) | Taint boundary, output guardrail, subagent model safety (ADR-0013/0015/0017/0019/0028), a quoted injection replayed by the plain history window, obeyed 2 of 10 on a bare turn and 0 of 10 behind either standing rule, the plain one landed for the tool-less turn (ADR-0013/0038), and, as of 2026-08-08, three guardrail bypasses fewer with the **count deliberately unmoved**, since none was ever counted: pricing the encodings tail against the shipped module found a URL spelled in the fullwidth and CJK twins of its own punctuation, a CJK-dotted host leaking past the default policy (the resolver reads `。` as a dot, `encodings.idna.dots`, and NFKC maps `｡` onto it rather than to a dot) and a fullwidth scheme separator (`https：//`) anchoring no match at all and so invisible to both policies, which is the seventh addendum's severe shape again; then the bracket-less entity colon that pass had deferred (`https&#58;//`) went the same way, closed as a whole family generated per character from its codepoint (decimal, hexadecimal, padded, semicolon-less, named, both separator characters, ten of the eleven spellings never named before) once its layer was distinguished: a renderer resolves an HTML reference before anything looks for a URL, while a source-code escape is resolved by a compiler that is not in the picture and a bracket-less percent-escape by nobody at all. Then on 2026-08-10 that whole table was priced by one question asked of the shipped path, **is there a resolver here that turns this spelling back into the URL**, which the traced path answers plainly (nothing downstream of the filter resolves anything and the overlay never makes a URL clickable, so the resolver is the browser the reader pastes into, and it was run rather than argued): the JSON-escaped slashes closed, since a URL parser skips `/` and `\` alike in a special scheme's authority and so `https:\/\/evil.example` IS the link, anchored nothing, and left the ledger empty when untrusted content spelled it that way; the source-code escapes, the percent-encoded scheme and the stacked references decline with their resolver named (a parser reads `evil\u002eexample` as the host `evil`, throws on a percent-encoded scheme, and needs a second rendering pass for `&amp;#58;` that the email sidecar's own unescape already supplies to a grammar that catches it), the trigger on the first being a Markdown renderer the overlay does not have. The tail stays open, now on its class rather than on any named row, and that run also found a live spelling on none of its rows and **opened it as an entry rather than chasing it**, which is the one thing here that moves the count: a special scheme whose authority carries fewer than two slashes (`https:evil.example/pay`, which a parser resolves to the plain link) anchors nothing under either policy, and admitting a separator that is *absent* needs a host-shaped lookahead the anchor has never needed. **That entry closed 2026-08-11 in the shape it predicted, and it is the one thing here that moves the count back**, since it was counted from the day it opened: the matcher's separator now spends at most one solidus behind a lookahead at the host, and the rule is that a host is a dotted name (every registrable domain, IPv4 literal and IDN, the dot counting in each reading the resolver has, so the label separators the identity folds by are imported into the grammar, joined by the HTML references a rendering pass resolves and by the one percent escape a parser decodes inside a host, `%2e` but not the stacked `%252e` it refuses) or a bracketed literal carrying a colon (an IPv6 address, where `[1]` and `[abc]` are refused here because a parser refuses them). The false-positive budget is the single label and it is spent where prose lives, so `https:scheme`, `http:foo` and `https:localhost` stay prose along with everything carrying a space, and the decline costs no exfil vector because no public suffix makes a bare label registrable. Two things came with it: the separator is composed out of the opaque family, so a defanged bare colon reaches the position too (`http[:]evil.example`) instead of repeating the bracket asymmetry, and the identity's authority slash run went from `+` to `*` so the empty run folds, without which the grammar would anchor a spelling the default policy still held no identity for (ADR-0015) | 12 |
| [memory.md](memory.md) | Store, scoping, rerank/MMR, the ranked `select` and its recall trail, and, since the user asked for the end-to-end turn cost before calling it and got 0.515 s of time to first token against a control whose interval spans zero, the judge's default no longer among them: `CORTEX_MEMORY_RECALL` ships as `judge` from 2026-08-08 and `raw` is the opt-out. What is left of the two the ranked-recall close opened and neither this cell nor the area header picked up until 2026-08-06 is the cross-encoder rank: the audit of the candidates a rank drops closed 2026-08-09, ahead of its trigger and because the default had moved under it, a judge keeping about one note where the cosine kept five having left the trail thinnest where most of the pool now disappears. The line names every passed-over candidate by id and by the store's cosine, bounded at the width a default deployment fetches and counting anything past it, so a memory that never came back stops reading like one the store never offered; it carries no key for a drop, a rank having an opinion on record only about what it kept. That question its own line raises next, why an id is in neither list, closed 2026-08-10 and took the count from 8 to 7 with nothing opening in its place, **ahead of a trigger neither arm of which fired**: no investigation had run and no pool had been widened, and it was taken because the entry's only blocker was the trigger rather than a cost argument. `MemoryStore` gained `count_candidates`, so the line now says what the pool was drawn from beside what came back and equal numbers mean the pool WAS the whole readable store. An exact count is the cheap design rather than the expensive one, an index-only `count(*)` costing 2.0 ms against a 520 ms search while folding the total into the ranked select would have cost 2.85 times it, and the entry's claim that the requested width is derivable from config is false under `raw` and on a fallback line, which costs no field because the count makes the width redundant. The gap the abstention close named rather than shut, a geometric policy that cannot decline, is no longer among them: it closed 2026-08-08 as **declined on measurement**, its second trigger having been a calibration giving the floor a number and the calibration having found that none exists, since answerable and unanswerable questions overlap on cosine behind both embedders the repo ships a path for and every floor that silences the one silences the other, worst of all in the vocabulary trap the model rank exists for. Its consumer was bigger than its own text said, the default deployment's own fallback being the cosine, which is why it was measured rather than shrugged at (ADR-0008/0038). **Eight from 2026-08-11**, when the `Embedder` port's shared check list established that both implementations raise `EmbedderError` and nothing else, and that nothing in the brain catches it or the store's error either: a stopped embedding server or an unreachable Postgres fails the turn instead of costing it its recalled notes, which is the opposite of how every other optional capability here degrades. **That entry closed the same day, hours after it opened and ahead of a trigger neither arm of which fired**, taking the count back to 7 and then to 9 on the two it opened, both residue of the close rather than found beside it. It was re-derived by running before it was believed, a turn with the embedder told to fail answering `TURN FAILED with EmbedderError` where the same turn answered in four events. Both halves degrade and for opposite reasons, which is what the entry's hardest question came to: the read because a turn genuinely has an answer without its notes, the write because raising cannot save it, the reply having streamed and the assistant message being persisted before the exchange is recorded, so an exception there loses the memory anyway and takes a read turn with it, and what is lost is a derived index entry rather than the exchange, which stays in the conversation. Exactly `EmbedderError` and `MemoryStoreError` degrade and the line is the adapters' own wrapping, so a policy's `ValueError` still fails the turn, and the catch sits in the two core functions the deep model's phase shares with the cortex's rather than in an adapter that also serves the delete cascade. The report is unconditional on the module logger rather than a line on the opt-in trail, and the trail's gain is an omission: nothing is written for a recall that never happened, so `pool == available` keeps meaning the pool was the whole readable store. The user hears once, about the read only, on the channel a fold narrates itself on. The two that opened are that the adapter wraps a malformed row into the same error an unreachable server raises, so a data defect now degrades wearing an outage's clothes, and that the `MemoryStore` shared check list still has no backend-failure check where the `Embedder` list has one (ADR-0008). **Still 9 after the ANN index was measured the same day and narrowed rather than closed**, which is named because a count that holds hides exactly this: the entry's whole reason, that an exact scan is fine at personal scale, is now half false, the shipped search costing 21 ms over a thousand memories and 1,478 ms over 220,000 against a 0.515 s recalling turn, so it is fine for months and dominant after years. `hnsw` is 268 times faster and charges the dimension-agnostic column ADR-0004 decision 4 keeps, plus a 0.550 overlap with the exact answer whose worst query kept none of its twenty; the entry survives because set overlap punishes a reordering among the 860 near-tied neighbours the corpus put around each query, so the right measurement is a score delta and that is the new trigger, `SET STORAGE PLAIN` having been measured too and buying only 22% (ADR-0004 ANN-index addendum). **Eight later that same day**, when the second of the two the unavailable-memory close opened shut ahead of its trigger, neither arm of which fired: the `MemoryStore` shared check list gained the backend-failure check the `Embedder` list already had, an eleventh check over a `MemoryStoreUnderTest` pair that walks `add`, `search`, `count_candidates` and `delete_scope` against a store whose backend is gone and requires `MemoryStoreError` from each rather than the driver's own type. What moved it is that the degradation the earlier close installed rests on that guarantee, so leaving it unheld is the gate that cannot fail. The real arm fails for real, `break_backend` being the adapter's own `aclose`, so asyncpg raises `InterfaceError('pool is closed')` inside every verb and `PgVectorMemoryStore`'s own wrapping is what the check reads; the cost is a store per check in the live driver, one check ending by destroying its own. Neither implementation leaked, so this records a guarantee held rather than a defect fixed, and it was proven able to fail on each arm alone, the fake's guard raising `RuntimeError` and the adapter's `search` narrowed to `PostgresError`, both restored. Eight is read by naming them, four of the eight being sub-items of landed entries rather than bullets of their own: the session and global union read policy, per-scope retention and eviction, cross-scope recall ranking, tiered and self-editing memory with summarization, the write-salience policy, the ANN index, a cross-encoder rank, and `MemoryStoreError` covering an outage and a malformed row alike (ADR-0008). **Still eight hours later that same day, by exchange rather than by standing still**, when the last of the two the unavailable-memory close opened shut ahead of its trigger too, no malformed row having been met and no other port having drawn the line: `MemoryDataError` subclasses `MemoryStoreError`, the adapter raises it from its decoding catches alone, and `_recalled_context` names it ahead of the degrading catch and re-raises, so an outage still costs a turn its notes while a row that will not decode fails the turn. The criterion is whether the condition heals with nobody touching the deployment, which a stopped server does and a corrupt row never does, so the alternative of degrading on both and logging the second loudly was refused on the previous close's own words, that a failure only a log records is the silence the degradation was written to end. The adapter can tell the two apart where it wraps rather than guessing, asyncpg's own family against a `KeyError` out of the row mapping, and the shared list holds only the half both implementations can answer, that a gone backend must never arrive as the subclass, since the in-memory twin decodes nothing. What opened in its place is the third call site the close deliberately left alone, `SessionServicer.DeleteSession` aborting `UNAVAILABLE` for the subclass too, so a cascade meeting an undecodable reply would offer the user a retry for a condition no retry improves. Eight is read by naming them again, the seven above unchanged and that seam mapping in place of the entry that closed | 8 |
| [inference-model-manager.md](inference-model-manager.md) | Model-manager lifecycle, MTP, reasoning status, whose disable-thinking and token-cap halves now reach every pass that discards its own deliberation, leaving the user-facing reply as the whole of that entry and the count unmoved for a narrowing (ADR-0007/0020/0038), and, since co-residency closed 2026-08-07 on a card that could finally test it, the two that close opened, of which the first closed hours later the same day: the co-resident deployment's fit is now measured against the card at the one instant it can be, immediately before the load, and the placer's budget stopped naming a cortex the handoff evicted the same day, charging the deep tier's declared cost at the two edges of the swap instead, which leaves the half neither a room check nor an epoch can see, a handoff that spilled anyway and whose only witness is decode rate, landed 2026-08-08 in the shape its own entry proposed and measured on the card at 20.38 to 22.77 tok/s spilled against 31.08 to 33.78 alone, both tiers reporting `ready` throughout, opening the two it declined: a handoff that has watched itself spill and still promises co-residency, and prefill as the second witness, and, one fewer since 2026-08-09, the deadline pairing that used to be written in three places and checked in none: the sidecar now reports the probe timeout beside the two stop bounds, the three ride one core value, and the brain refuses to serve when its own control deadline does not strictly clear their sum, one out and none in because the staleness that leaves (a sidecar restarting under a running brain with changed env) has the same cause and the same fix as the residency entry it was folded into, which landed hours later the same day and took both halves with it: the daemon names its own boot on that same route, the brain records which one it was told at startup and asks again before each handoff evicts anything, and a different answer converges residency, republishes what convergence observed and re-reads the stop bounds, the count holding at 7 because the reconciliation is reachable only through a handoff and the two report staleness cases with no restart behind them are precisely the ones a brain with nothing resident cannot reach (ADR-0030) | 7 |
...
| [resource-governance.md](resource-governance.md) | Scheduler/placer budgets, NPU, drain (ADR-0012); the cortex reservation was re-measured on 2026-08-07 at the tier's shipped shape and lowered from 11.3 to 8.6 GiB, and the placeholder that correction uncovered was measured the next day and closed, the subagent VRAM ask landing at 3.5 GiB in both of its declarations, so the shipped stack GPU-places a spawn for the first time and every term of that budget is now a measurement; and the read timeout this area called the actual unbounded-wait hazard landed 2026-08-09 on **two** clients rather than the one its entry named, the resident tier's carrying the same `read=None` and the deep model streaming through that very object after a handoff, as a per-read stall ceiling (120 s resident, 600 s pool) that bounds the gap between chunks and never a generation's length, leaving the count unmoved because the same pass opened the failure a stall detector cannot see, a subagent that keeps talking; and hours later the bounded admission wait landed too, ahead of its trigger and because that ceiling had removed the excuse, `admit` now refusing after `CORTEX_SUBAGENTS_ADMISSION_WAIT_S` (3600 s, twice the 1800 s the last spawn of a full batch waits when an entry's admitted pair serializes on one placement target and four times the 900 s it waits when the shipped placement overlaps that pair, an upper bound over both rather than the equality the derivation first claimed and a correction that took two passes to reach every restatement) with the same typed error the runner already degrades, its port opened and read before anything was designed and found genuinely unchanged this time, and the count unmoved a second time because the queue-depth half it declined took its place; and hours later again **admission reopening onto a tier the swap back could not restart** landed too, ahead of its trigger and with the port question going the other way for once, since `place` is synchronous and argument-poor so nothing can ask it whether a tier is up and the only shape that fits is a verb: a peer the restart could not bring back is recorded in `StandingTiers`, which closes GPU placement (`close_gpu`/`open_gpu`, a second pair kept independent of the handoff charge precisely so a successful swap back cannot silently reopen an outage), names the tier on a **serving** `Health` reply that the overlay already renders as `Brain ready: <line>`, and is retried every `CORTEX_SWAP_TIER_HEAL_S` until a pass sees it ready. Its own proposal to widen `ResidencyReport` was wrong on a lifetime rather than a shape, that value being republished at every residency transition. One out and three in, so 5 becomes 7; and hours later one of those three, **boot recovery calling a peer tier's failure the cortex being gone**, closed as well, ahead of its own trigger, with its stated blocker still exactly true (`residency.py` at 299 of 300, so the fix needed a split, and `ResidencyBoard` is that split taken by responsibility: the bookkeeping the moves and the restore both publish into). What its text could not know is which call a real deployment fails at, and a real sidecar over real HTTP is what said so: a tier named in `CORTEX_SWAP_EVICT_MODELS` with no artifact is not in the daemon's roster, so it answers 404 to the **status** the clearing loop asks first, several calls before the `start` the entry named, and the boot answered `settled=False` while the cortex read `ready` on that same daemon. Both ends of the convergence are peer-tolerant now and the verdict is the cortex's alone, which also cost the detail line its cause clause, a tier having no deep task to blame on a brain that never escalated. One out and one in, the deep tier's own clearing, so the count holds at 7; and hours later again the **drain bound against a fired task's lease** closed as declined with nothing taking its place, the first departure in this area with no arrival beside it, because its premise did not survive the tree: `drain` waits on `_in_flight`, which `admit` holds around a whole subagent run, while the schedule lease is the ticker's `wait_for` cancellation cap, so the entry's 60 s against 300 s compared a wait bound to a ceiling and the number the drain is really up against is the measured 200 to 300 s subtask, which makes the abort likely rather than the systematic the entry claimed. Both knob moves it proposed are refused (lowering the lease cancels fires before their own subtask finishes; raising the drain bound covers fires and not the interactive spawns that hold the same admission with no lease at all and no cap on a generation's length either, and the smallest value that even covers a wedge sits above the 600 s ceiling, which is the handoff latency the default was chosen against), so what landed is the falsified rationale: the comment on `DEFAULT_SWAP_DRAIN_TIMEOUT_S` and its restatement in the core module doc had called 60 s generous enough for a normal delegated run to finish, which this repo's own measurement denies. 7 becomes 6; and the **total generation cap** landed 2026-08-11 ahead of its trigger, the fix-when-it-bites entry whose blocker was never the mechanism but the guess about how long a legitimate answer runs, which turned out to be an afternoon of measurement on the shipped CPU entry rather than a guess: five subtask shapes from a one-word lookup to an open-ended essay, and a cap of 1024 tokens sized at roughly five times the longest narrow reply against a deadline of 2400 s at four times the longest whole subtask, the extra doubling covering a tool-using run whose loop spends on several rounds what the measurement spent on one. What landed is `AttemptBounds(max_tokens, timeout_s)` on the runner, the cap riding every completion an attempt asks for and the deadline an `asyncio.timeout` around the whole consumption so it covers the tool dispatches between them, reported as `AttemptFailure.TRUNCATED`, an `ok=False` result naming the bound and deliberately not re-placed on the CPU for the reason a malformed reply is not. Its own "expressible today" was true of the port and false of the path, `ToolLoopContext` having no `bounds` field to carry a cap by, which one field of loop vocabulary fixed with the port genuinely untouched; and two questions it could not have asked are settled rather than left implicit, the deadline being per attempt rather than per task and having to outlast the stall ceiling, which the config now refuses to start without. Two take its place, a finish reason the port does not carry so a capped completion looks like a finished one, and the 200 to 300 s whole subtask two derivations rest on being out by a factor of two for a summarization. One out and two in, so 6 becomes 7; and later the same day **the deep model's clearing deciding the cortex's verdict at boot** closed as well, ahead of its own trigger and built exactly as its text specified, the port gaining the narrower failure it named (`ModelNotHostedError`, a subclass so nothing that catches the broad one notices, raised for a 404 on a per-model route and for nothing else, since the daemon has drawn that line since it was written and only the adapter was discarding it). Its account of the tree held to the line, which is what made this small, and it understated its own defect twice over: an unrostered deep tier was not only an amber dot but, one call further along, the loss of the assistant, because the swap back's stop of the model it swapped in met the same 404, failed the restore, failed the retry, and raised `ResidencyRestoreError` with the cortex left stopped and the seam saying recovery was manual, all witnessed against a real supervisor over real HTTP before and after. The boot verdict is now green over a serving cortex with one loud line naming both knobs, while a deep model that is resident and will not stop, an unreachable sidecar, and a cortex the roster does not have stay amber, which is the direction that would have been worse to get wrong and is what three of the seven measured mutations pin. One takes its place, the brain learning that escalation cannot work and then forgetting it, so each attempt still drains, evicts and reloads the cortex to rediscover the same 404, and the count holds at 7; and hours later again **the retry that only asked about tiers it already believed were missing** closed too, ahead of its own trigger, with its five shapes driven against a real supervisor over HTTP before anything was designed and the count coming out four rather than five: the accept-then-die, the quiet death between handoffs, the peer nothing started because a convergence returned before its restart loop, and the boot that could not reach the host all left a spawn landing on the GPU at a dead endpoint with the record empty, while the unrostered tier the entry added last was already closed correctly and merely noisy, at one control call a pass rather than the two its text claimed. So a pass now reads `status` for every `CORTEX_SWAP_EVICT_MODELS` tier and writes what it hears (`residency_sweep.py`), the record turning from a list of refusals into a cache of the machine refreshed every interval, which is also the argument for it staying in the process; and it carries `TierFault` per tier so an unhosted one is recorded once, skipped for ever, and left closed at the placer, its only clearing path being the daemon replacement the boot watch already notices. The risk the entry deferred on is answered rather than assumed away: "only a refusal marks" becomes "only an observation taken outside a handoff marks", the fence widening to the handoff claim as well as the residency scope so a pass stands down from before the drain, and both flags re-read synchronously in the instant before every start. One takes its place, the start already on the wire when a handoff begins, so the count holds at 7 | 7 |
| [email-confirmer.md](email-confirmer.md) | Email write, Confirmer, attachments, `ToolActivity` chip (ADR-0022); the **per-field attachment schema descriptions** closed 2026-08-11, the smallest thing this backlog held and one whose entry had never been read against the code, and the reading is what made it work rather than decoration: the object was described and its three fields carried `title` and `type` alone, so `content` beside `filename` read as a location beside a name and a path written into it composes, sends and arrives as a file whose whole text is that path, refused by nothing; `subtype` invited the very `text/markdown` its token rule bans; `filename` carried an unguessable 128; and the array, whose two bounds belong to no field of the object, had no description at all. Every one of those is refused inside the sidecar, after the gate and after the user approved the card, so the entry's own "to say what the type names already say" was the half-sentence that had kept it open, and its other objection, pydantic in the pure values module, was answered by its own finding that the module has been a prompt surface since the docstring was first lifted from it. The three bounds moved there with the prose, so the number the model is told and the number the sender enforces are one value read twice. One out and one in, the sibling sweep having found the read half's `query` unstated as a dialect and left it deferred for want of a live pass, so the count holds at 4 | 4 |
...
| [vision.md](vision.md) | Screen capture, images, the pixel boundary (ADR-0029), one fewer since 2026-08-08 when `RESOURCE_EXHAUSTED` classification closed whole: the arithmetic the 2026-08-06 re-read published still holds, `CaptureError::TooLarge` being unreachable at the shipped byte ceiling at any edge the seam permits, but the wording defect that re-read folded in is reachable on an untouched install and was the trigger nobody had counted as the trigger, so the body's codes and the brain's reading of them landed together (ADR-0023). **Eleven still on 2026-08-10**, read entry by entry rather than carried forward, when region and window capture landed its body half: the seam carries a `CaptureTarget` and a shipping body honours it, since under proto3 a field no body reads is a silent lie rather than a spare knob, but nothing asks for a window yet, so the entry is half closed and a half close moves no count. The model-named rectangle was declined inside it and stays uncounted until an overlay-drawn region picker makes the rectangle user-authored. **Eleven still, later the same day**, when the brain half landed too and the entry stopped being work: the tool takes a required target, the model picks it from a schema derived from the domain enum, the reply says which of the two things the picture is (read off what was encoded, so it and the OS receipt cannot disagree), and `describe()` stops calling a crop a shrunk screen. The entry's **name changes rather than leaving**, to the measurement the whole thread was for, whether a window-sized crop reaches the 15 px text that stayed at 4 of 16 at every token budget tried, so one name is out and one in on one bullet and the count holds, re-derived entry by entry in the area doc rather than carried. Two things the body half's own handover list did not have: the honest sentence needed a reply-side field, since a crop and a shrunk screen are the same blob with the same source size; and the free repeat bound became two captures per target and four a loop, four rather than six because a call naming no target is refused before the body is reached and four rather than unbounded because the vocabulary is matched exactly. One input to the ungated decision moved without moving it, a confirm card now having a target it could name, recorded at the ADR for the maintainer to overrule knowingly. **Ten on 2026-08-10**, read entry by entry again rather than decremented, when the measurement that entry had become was run: five desktops and 47 ground-truth strings through the body's own crop and downscale, both arms in one session at the shipped budget and capture edge, three runs. The 15 px row the entry was named for goes from a flat 5 of 12 on the shrunk screen to 9 or 10 of 12 on the crop, and the terminal at 100% scaling is 2 of 7 against 5 of 7 in every run with the shrunk screen declining where the crop transcribes exactly. It closes on a measurement rather than on a win: over all 47 the crop reads worse (29 to 31 against 32 to 33) because it cannot see what is outside the window, everything above 15 px is level, and the one window wider than the capture edge is resampled like the screen and reads no better, so the mechanism is being unresampled rather than being cropped. The corpus had to be rebuilt, the recorded one having been a scratch harness, so the control was re-run rather than cited and no number is comparable with the recorded table; the transcription of the downscaler was proven equal to the Rust on four cases rather than eyeballed. **Eleven later the same day**, this area's first arrival since the swap entry's halves and the first reading here that moves a count up, when the shipped steer was corrected against that measurement and raised a field it deliberately did not build: the tool description had promised the model that a focused window is cut out "at full detail", unconditionally, and the mechanism is being unresampled rather than being cropped, so a window wider than the capture edge gets the same box filter and the same 2048x1152 the screen does and is indistinguishable from an untouched crop on arrival. The description now names small text in one thing as the case the window wins, says what it costs (everything outside that window) and promises no detail it cannot keep, and both copies of the steer are held to that by a test proved able to fail three ways. **Whether the reply says a window was resampled** is the new entry, deferred rather than built because its only consumer is a caption in the stand-in text and that is the intervention this area has measured twice and found inert, while the half the model can act on landed in the description, before the pick rather than after the picture. One restatement went with it: refusing a call that names no target had leaned on the whole screen being the less legible picture as well as the more exposing one, which the measurement narrows to the smallest type alone, and the refusal never needed that leg | 11 |
...
a read timeout on the subagent HTTP client) that name the waits nothing bounds. **Both of those
landed on 2026-08-09**, hours apart and in that dependency order, so the sentence before this one
is history rather than a description of the tree: what is left of that pair is the queue-depth
refusal the wait bound declined and the runaway generation the stall ceiling cannot see. Body gateway held
...
went 5 to 6 with **the drain bound against a fired task's lease**: the shipped defaults were read
as making a handoff requested during a scheduled task abort every time (correctly, before evicting
anything), which was filed as a defaults decision to make against real usage rather than a design
change. That reading was traced to the code on 2026-08-09 and declined, the drain waiting on an
in-flight admission rather than on any lease, as the row for that area records.
...
[ADR-0038](../adr/ADR-0038-ranked-recall.md)). **That entry closed on 2026-08-08 as declined on
measurement**, taking memory from 9 to 8, and the threshold this line worried about turned out not
to be the binding problem: no threshold separates answerable questions from unanswerable ones behind
*either* embedder, so the floor fails before portability is reached. That is memory's second close of
2026-08-08 and the second with nothing opening behind it, after the judge's default moved the same
day; the abstention close the day before is the one that opened something, and what it opened is
this.
...
Resource governance went 5 to 7 on 2026-08-09, the third of that area's closes that day and the
first to open more than it shut: **admission reopening onto a tier the swap back could not
restart** landed ahead of its trigger. The entry's own account of the code was re-derived first,
as the warnings above demand, and it held on three points and moved on two. It held that the
restart really is best effort, that `undrain` really does reopen on every path, and that
`ResidencyReport` really carries no per-tier state. It moved on the port, in this area's
characteristic direction: `place` is synchronous, lock-free and argument-poor, so a placer cannot
be *asked* whether a tier is up and the only shape that fits is being *told*, which is a verb and
therefore a port change (`close_gpu`/`open_gpu`, deliberately a second pair rather than a cleverer
charge, since a resident charged large enough to crowd the cap out would say "no room" where the
truth is "no server" and the next successful `charge_standing` would silently reopen the outage).
And it moved on the fix itself: widening `ResidencyReport` would have been wrong on a lifetime
rather than a shape, that value being republished at every residency transition, so a peer's
down-ness written into one would be dropped by the next swap in. The record lives beside the
report and is folded in on read. The distinction the entry never named is the one the design turns
on, down versus merely evicted, and it is three mechanisms rather than a comment: only a `start`
that raised marks, only a serving report is annotated, and the handoff window is covered by the
drain and the charge instead. What the user sees needed no new surface at all, `HealthReply`
already carrying a detail beside `ready` and the overlay already rendering a ready one as
`Brain ready: <line>`. The failure side was witnessed against a real `model-host` container over
real HTTP, both a tier the daemon refuses outright and one that accepts a start and dies, and it
is that second measurement that opened the first of the three entries this close leaves behind
(the retry only asks about tiers it already believes are missing; the placer holds one bit for the
card; boot recovery still calls a peer's failure the cortex being gone). One out, three in, which
is the backlog working as intended: the record the entry wanted is built, and what replaces it are
the questions building it made askable.
...
- **The abstention the judge can reach and its policy cannot report**
  ...
  deployment on any geometric policy still gets its nearest misses. **That entry closed 2026-08-08
  as declined on measurement**, so this thread is finished: the relevance floor it proposed was
  calibrated on the real embedder and no threshold separates the answerable questions from the
  unanswerable ones, which makes declining a property of reading rather than one geometry can be
  tuned into.
...
- **`bridge/demoBridge.ts` stays over that cap** ([body-overlay.md](body-overlay.md)), open from
  ...
  **Its last live sentence expired 2026-08-11**, when the trigger it set, the demo growing a second
  behaviour worth testing, fired: the demo bridge joined the overlay's shared `BrainBridge` check
  list, both it and its script came out of the coverage `exclude`, and the script's every line is
  reached by the turns those suites drive, so the 0% measured above was about a script nothing
  imported in CI rather than about the file ([repo-gates.md](repo-gates.md)).
...
- **Region or window capture** ([vision.md](vision.md)): the fix for the vision slice's headline
  ...
  **Closed later on 2026-08-10, on the measurement rather than on a win**, the brain half having
  landed in between and left the measurement as the only thing owed. The 15 px row goes from a
  flat 5 of 12 on the shrunk screen to 9 or 10 of 12 on the crop over three runs, and the terminal
  at 100% scaling is 2 of 7 against 5 of 7 in every one of them. Against that: over all 47 strings
  the crop reads worse, because it cannot see the five outside the window; every size above 15 px
  is level; and the one window wider than the capture edge is resampled exactly as the screen is
  and reads no better, so what does the work is being **unresampled** rather than being cropped.
  The count moves 11 to 10, and the corpus is in the tree this time
...
cost-aware batch cap and the fair-share policy, alongside which the sidecar session cache/pool sat
until 2026-08-08 on its own entry's word that the per-call handshake was acceptable at personal
scale, an adjective it never converted into a number and which was **struck** the day it was
measured: the open is 17.8 ms on the transport this repo's own sidecars serve, 0.4% of a recalling
turn's time to first token, while the 565 ms the reference filesystem sidecar charged for the same
open turned out to be that sidecar spawning an `npx` process per JSON-RPC request and leaking every
one of them, so what the entry had blamed on the client was fixed in the compose command and the
pre-token walk went from 1156 ms to 146 ms with no brain code touched, leaving as the reopen trigger
only the residual a held session would remove and the port change across seven combinators that
holding one would cost ([tools-mcp.md](tools-mcp.md)); the
...
possible, whose trigger was either side's timing being tuned or a handoff aborting on an eviction
...
out not to transfer, that check having landed at the swap instead, **struck 2026-08-09** when this
one landed ahead of its own trigger, its account of the code re-derived first and held: the third
term joined the health body, the three became one core value with the rule as a method on it, a
fifth `ModelHost` verb reads them back, and the composition root refuses to serve on an answered
mismatch while an unreachable sidecar stays tolerated exactly as boot recovery already argued it
must be, since the price the entry named turned out to be a dependency that already existed and
declines to raise ([inference-model-manager.md](inference-model-manager.md)); noticing a handoff that spilled
...
trigger was a deep phase that is slow rather than absent and the fix was the deep phase watching
llama.cpp's own `timings.predicted_per_second`, which the backend discarded, **struck 2026-08-08**
when that watch landed ahead of its trigger and in the shape the entry proposed, its account of the
code having been re-derived first and held, leaving two entries behind it, a spill that does not
change what the next handoff promises and a prefill rate nothing reads
...
footer of short words were all rejected (measured against the shipped gate: three complaints and
exit 1), whose trigger was the first commit that genuinely needed a command or a block in its body
and which was **struck 2026-08-09** ahead of that trigger, no such commit existing in 433: the wrap
walks the message carrying a fence toggle now, steps over a fenced line and over a `$` prompted
paste, and reports a fence left open rather than letting one exempt every line after it. The footer
was decided rather than exempted, wrapping like the prose it is, since git's trailer token admits no
space and so never reads it at all while the parser that does read it allows newlines in a footer
value; the entry's sharpest claim, that the gate could refuse a message the commit rules mandate,
did not survive being measured, a wrapped footer having always passed; and the leading-indent half
of its own heuristic was refused on this repo's history, where all 9 indented body lines are prose.
What replaced it is narrower and is the same exemption read to its edge: a paste is exempt from the
width and from nothing else, so a bare `--` separator or a hash that resolves is still refused
inside a fence, whose trigger was the first commit whose paste carries either, and which was
**struck 2026-08-09** ahead of that trigger, no such commit existing in 437 (the one prompt-marked
line in the history carries only attached flags and no hex) and the wall being one paste away
instead, since three of this repo's own gate invocations spell the separator. The answer is per
rule rather than blanket: the dash ban joins the wrap inside a paste, both being about the text as
typed and neither having a remedy there, a reflowed command and a stripped `--` saying something
else, while the volatile-reference ban and the resolving-hash check do not, being about the message
still reading correctly once what it points at moves, which does not care who typed the pointer and
whose remedy survives intact, `git show <sha>` carrying everything the paste carried. The entry's
own narrower reading, exempting cargo's `--` but not an em dash, was refused as a rule about
character sets rather than kinds, and one `classify_lines` walk now answers for every rule that
turns on a paste ([repo-gates.md](repo-gates.md)); the retry
...
([body-overlay.md](body-overlay.md)); the two bounds the panel's section budget left behind it,
joined on 2026-08-03 when the budget landed and both closed on 2026-08-08, are gone from this bucket:
a section's own border, padding and air sat under no cap at all, so two open sections cost 40px the
budget could not reach and the hint strip stood 34px outside a 640x240 viewport, and the share now
bounds the section's wrapper, which has no frame of its own to floor it, taking the shortest viewport
everything fits in from 286px to 220px (the entry's own boundary having been 284px rather than the
"roughly 260px" it named); and the room a closing section handed back arrived in a single frame,
because the share read the tree and a section rolling shut is in the tree until React removes it, and
the share now reads the target that roll publishes instead and both caps ease to it on the roll's own
clock, taking the switcher's largest single frame from 105.86px to 15.94px with a boundary step of
0.000px ([body-overlay.md](body-overlay.md)); the whisper's two bounded follow-ups, joined on
...
the ANN index, **measured 2026-08-11 and narrowed rather than closed**, its reason for existing
having been that an exact scan is fine at personal scale and nobody having put a number on personal
scale: the shipped search costs 21 ms over a thousand memories and 1,478 ms over 220,000, about
three times the whole 0.515 s recalling turn, so the claim is true for months and false for years,
while `hnsw` answers the same search in 5.5 ms and charges for it twice, freezing the column at one
embedder width and overlapping the exact answer only 0.550 with one query keeping none of its
twenty. The entry survives because that last number is the wrong measurement, set overlap punishing
a reordering among near-tied neighbours no reader would notice, so its trigger is now a score-delta
calibration and the unfireable "when it bites" is struck (ADR-0004);
**recall observability, which this bucket claimed until 2026-08-06 and which is
...
`RecallAudit` did not carry because a non-picked candidate's `SPREAD`/`SWEEP` key is not well
defined, whose trigger was the first investigation that needs to know why a specific memory was not
returned and which **closed 2026-08-09 ahead of it**, the default having moved to `judge` in the
meantime and a judge returning about one note where the cosine returned five, so the pool the trail
said nothing about became most of it. The obstacle answered itself once it was read closely: no
key for a passed-over candidate is on record under any basis, a `Ranking` carrying keys for the
hits it kept and for nothing else and the judge leaving an unhelpful note out of its order rather
than scoring it low, while the store's cosine exists for every candidate because the store produced
the pool, so the line logs the id and that cosine and omits the key that does not apply. It is bounded at the width a default deployment fetches, with a count of anything past it so
a truncated list is never read as a complete one, and it is assembled inside the sink guard so the
opt-out still pays nothing. In its place, from the same close: **why a memory was never a
...
makes the entry smaller and not moot), whose fix was to have been a relevance floor on
a fifth policy (not on `RawRecallPolicy`, whose promise is byte-for-byte v1 recall) and whose trigger was a
deployment wanting recall to stay geometric and still say nothing, or a calibration giving the floor
a number that means something behind more than one `Embedder`, and which **closed 2026-08-08 as
declined on measurement** when that calibration ran and found no such number: the answerable and
unanswerable populations overlap on cosine behind both embedders the repo ships a path for
(separation 0.1582 and 0.1933 negative), the tightest floor that silences all four unanswerable
questions costs 6 of 22 answerable ones and drops the vocabulary-trap category from 0.81 to 0.17,
and the widest band that costs nothing is two hundredths wide and is a sample minimum rather than a
bound; the re-derivation also found the consumer to be the default deployment rather than an opt-out
one, since the shipped judge's fallback is the cosine, and the run ships as an integration-marked
test whose finding assertion reddens behind an embedder that does separate
([memory.md](memory.md)); the guardrail tails, four of them since 2026-08-11 (whitespace-split
hosts, full UTS-39 confusables, further encodings, footer heuristics), where five stood the day
before and the fifth was a slashless authority URL,
of which **further encodings was priced
on 2026-08-08 and stays open with a table**, the run finding source-code escapes
(`\u002e`, `\x2e`, `\056`, `%u002e`, `\.`) unfolded and JSON-escaped slashes, a whole
percent-encoded scheme and a bracket-less entity colon (`https&#58;//`) anchoring nothing, all
deferred on the line that pass drew (a renderer or the resolver undoes every obfuscation closed so
far, while a source-code escape asks the reader to decode by hand), and the same run turning up two
bypasses that were **not** in the tail and landing them: a CJK-dotted host that leaked past the
default policy and a fullwidth scheme separator that matched nothing at all, so neither policy saw
it; the entity colon then came off that table the same day, its whole family generated from each
separator character's codepoint, on the layer distinction the line needed (a renderer resolves an
HTML reference before anything looks for a URL, a compiler that is not here resolves a source-code
escape, and nobody at all resolves a bracket-less percent-escape); **that leftover table was then
priced whole on 2026-08-10 and the tail stays open on its class**, the pass deciding every row by
one question put to the traced path (nothing downstream of the filter resolves anything, the
overlay renders reply text as a React text child and makes no URL clickable, so the resolver is the
browser the reader pastes into, run in `node` rather than argued): the JSON-escaped slashes closed,
a URL parser skipping `/` and a backslash alike in a special scheme's authority, so that spelling
IS the link, anchored nothing under either policy, and left the ledger empty when untrusted content
wrote it; the source-code escapes, the percent-encoded scheme and the stacked references decline
with their resolver named, and the identity now folds a special scheme's backslashes so it agrees
with the parser that an escaped host is the host `evil` and not the link; **the same run found a
live spelling on none of the table's rows and opened it rather than chasing it**, a special scheme
whose authority carries fewer than two slashes (`https:evil.example/pay`, resolved to the plain
link, anchoring nothing under either policy), which is the one item here that moved a count, since
admitting a separator that is absent needs a host-shaped lookahead this anchor has never needed;
**that entry closed 2026-08-11 and moved the count back, the only one of these tails to have both
opened and closed inside the order it was written into**, and it closed in the shape it predicted
rather than in the shape a widening usually takes: the anchor grew its first lookahead at what
follows a separator, holding a slashless authority to a host, and a host is a dotted name (every
registrable domain, IPv4 literal and IDN, the dot spelled by the label separators the identity
already folds by, by the HTML references a rendering pass resolves, and by the one percent escape a
parser decodes inside a host) or a bracketed literal carrying a colon (an IPv6 address). Its budget
is the single label, declined because `https:scheme` is both a live URL and how a sentence names a
scheme, and declining it costs no exfil vector since no public suffix makes a bare label
registrable; the separator was composed out of the opaque family so the defanged bare colon came
with it, and the identity's slash run went from `+` to `*` so the empty run folds
([untrusted-content.md](untrusted-content.md)); the GBNF alternative, the
...
([untrusted-content.md](untrusted-content.md)); **per-field attachment schema descriptions, which
landed 2026-08-11 and were replaced in this bucket the same day by the read half of the same
defect**, the smallest entry this backlog held and one whose account of the code had never been
checked, so the close began by checking it: the object was described and its three fields carried
`title` and `type` alone, which leaves `content` beside `filename` reading as a location beside a
name, and a path written into it composes, sends and arrives as a file whose entire text is that
path, refused by nothing because a path is a good string, while `subtype` invited the very
`text/markdown` its token rule bans, `filename` carried a 128 nobody would assume from `str`, and
the array carried nothing at all though its two bounds belong to no field of the object. Every one
of those refusals lands in the sidecar, which is after the gate and after the user approved the
card, so a wrong guess is a send the user consented to and did not get rather than a cheap retry,
and that is what lifted the entry's own "to say what the type names already say" off it. Its second
objection, pydantic in the pure values module, fell to its own finding, the module having been a
prompt surface since the docstring was first lifted out of it; the three bounds moved there too, so
the number the model reads and the number the sender enforces are one value. What takes its place
is `search_emails`, whose `query` reaches imap-tools unaltered as raw IMAP `SEARCH` criteria while
the tool says only `an IMAP query`, so a model writes `from:someone@example.com`, the syntax of
every other mail client it has ever seen, and gets a `BAD` back; it is deferred rather than
described because an honest list is a list of criteria that work and this repo has run exactly two
against a real Bridge, `ALL` and `SUBJECT "..."`, so its trigger is either a deployment observed
failing a search or the one live pass over the criteria worth naming; and
...
timeout design over a `Clock`" (it is `asyncio.timeout`, the mechanism `drain` already used on
that condition, a duration belonging on the loop's monotonic clock while the `Clock`/`Sleeper`
pair exists for poll loops). Its port was opened and read before anything was designed, this
being the area whose blanket "behind the unchanged port" broke twice, and the signature really
was unchanged, gaining a contract sentence that an implementation which queues owes a bound on
that queue. What replaces it is **a queue-depth bound**, the early refusal to the wait bound's
late one, declined for now because the scheduler holds charges and no durations, so any depth is
a guess where the wait is arithmetic over measurements; its trigger is the first deployment
observed hitting the wait bound, which is also the first with a drain rate to derive a depth
from. Its sibling, a read timeout on the subagent HTTP client, **landed 2026-08-09 and was
replaced in this bucket the same day by what it could not see**, since the wedged
`llama-server` stream that entry named is now bounded on both generation clients (it named one
and there were two, the resident tier's carrying the same `read=None` and the deep model
streaming through that very object after a handoff) by a per-read stall ceiling that detects
silence, while a subagent in a repetition loop is never silent and holds its admission exactly as
the wedged one used to, so a **total generation cap** took its place with the first delegated
run observed running away as its trigger, a token cap already expressible through
`GenerationBounds.max_tokens` and the wall-clock cap that would actually bound the pool needing
the same timeout design as the admission wait beside it, which that wait then made cheaper than
this line first priced it by landing on `asyncio.timeout` and no injected `Clock` at all,
**struck 2026-08-11** when it landed ahead of that trigger: the blocker was never the mechanism,
both halves having already been priced down to nothing, but the guess about how long a legitimate
answer runs, and that is an afternoon of measurement on the shipped CPU entry rather than a guess,
five subtask shapes from a one-word lookup to an open-ended essay giving a cap of 1024 tokens at
roughly five times the longest narrow reply and a deadline of 2400 s at four times the longest
whole subtask, which also lands it between the pool's stall ceiling and its admission wait, so a
run can never hold its admission longer than a peer is willing to queue for it.
The trigger asked for one observed runaway to size from; what the measurements gave instead is the
other end, the longest run that must not be cut, which is the end a cap is actually sized against.
Its "expressible today" was true of the port and false of the path, `ToolLoopContext` having no
`bounds` field for a subagent's only route to that port, which one field of loop vocabulary fixed
with the port genuinely unchanged, this area's blanket claim coming out true for once and not for
the reason the entry gave. Two take its place, a finish reason the port does not carry so a capped
completion looks like a finished one, whose trigger is the first capped reply mistaken for a
finished one and whose fix really is a port change, and the 200 to 300 s whole subtask two
derivations rest on being out by a factor of two for a summarization, whose trigger is the first
spawn refused at the admission bound; joined on
...
can skip a downed tier by, not a scheduler change), **struck 2026-08-09** when it landed ahead of
that trigger: the peer is recorded in `StandingTiers`, the placer is **told** through a second
verb pair (`place` being synchronous and argument-poor, so "behind the unchanged port" was wrong
here in the direction this area keeps getting it wrong), a serving `Health` reply names the tier
on a surface the overlay already renders, and a retry loop clears the record the first pass that
sees the tier ready, with a real sidecar over real HTTP witnessing both the 404 refusal and a
start-then-die. Three take its place, each opened by building it: **a retry that only asks about
tiers it already believes are missing**, whose escaping shapes include the start-then-die just
measured and, from 2026-08-11, a peer the daemon has no artifact for, retried for ever against a
roster that cannot grow now that the port can say so, and whose fix is a sweep over every evicted
tier, triggered by the first deployment
running with a peer dead and nothing noticing, **struck hours later on that same day** when it
landed ahead of that trigger: all five shapes were driven against a real supervisor over HTTP
before anything was designed, four of them really did leave a spawn on the GPU at a dead endpoint
and the fifth never did, so what landed is a sweep for four and a retirement for one, the record
becoming a reading of the machine taken every interval instead of a list of refusals and carrying
a reason per tier so a roster that cannot grow stops being asked. The risk the entry deferred on
is answered rather than assumed away: the fence is the handoff claim as well as the scope, so a
pass stands down from before the drain, and both flags are re-read synchronously in the instant
before every start. One takes its place, the start already in flight when a handoff begins, which
the supervisor's per-model lock orders and which costs a refused handoff rather than a lost one;
**a placer holding one bit for the card** where
the record holds one entry per tier, for want of any declared mapping from a hosted tier id to
the GPU endpoint a roster entry dials, triggered by a deployment evicting more than the subagent
tier; and **boot recovery still calling a peer tier's failure the cortex being gone**, which is
the same conflation the close refuses everywhere else, left at that one site because both callers
reach the record through a file sitting one line under the cap, triggered by a boot observed
telling that lie, **struck hours later on the same day** when it landed ahead of that trigger: the
blocker was still exactly true, so the fix began with the split it named, taken by responsibility
rather than by count (`ResidencyBoard` is the bookkeeping the moves and the restore both publish
into, one object because the resident and the report are one invariant), and then went one call
further than the entry did, because a real sidecar over real HTTP showed the reachable
misconfiguration failing at the clearing loop's `status` rather than at the restart's `start`, a
tier with no artifact being absent from the roster and 404 to every verb; both ends are
peer-tolerant now, the verdict is what was observed of the cortex, the record is the manager's own
handed to boot recovery rather than a second one, and the detail line stopped naming a deep task as
the cause since a boot has had none. One took its place, **the deep model's own clearing**, which
was deliberately still fatal to that verdict and rightly so for an unreachable host and for a deep
model that really will not stop, but wrong for a deployment that turned escalation on without
naming `CORTEX_MODEL_FILE_BRAIN`, whose tier the daemon 404s for ever; the port could not tell
those apart, so the fix was a narrower failure on it and the trigger the first boot observed
amber over a serving cortex, **struck 2026-08-11** when it landed ahead of that trigger and built
exactly as specified: `ModelNotHostedError` is a subclass, so no caller that catches the broad
error notices, and it is raised for a 404 on a per-model route alone, the daemon having always
answered that case differently and only the adapter having thrown the answer away. Boot recovery
tolerates it for the deep tier and nothing else, so that boot is green with one loud line naming
both knobs while a resident deep model that will not stop, an unreachable sidecar and a cortex the
roster lacks stay amber. The entry had understated itself: the same 404 reached the swap back's
stop of the model it swapped in, failed both restore attempts and raised `ResidencyRestoreError`
with the cortex left stopped, so the misconfiguration cost the assistant rather than a dot, which a
real supervisor over real HTTP showed before and after. What takes its place is that nothing
remembers the line was logged, so every escalation still drains, evicts and reloads the cortex to
rediscover the same refusal, whose fix has to decide where a fact about the daemon's roster may
live on a brain that re-derives every other belief about that daemon whenever it is replaced, and
what the seam says about a capability that is configured and unavailable, triggered by a
deployment observed paying that stall
...
second capture encoding), **struck 2026-08-11** ahead of that trigger and with its account of the
tree checked before it was believed, the comparator being `Relation.MEMBER`, which reads registry
order like the ordering beside it so that every site but the last declares a value and the last
declares the collection carrying them, and the value form a one-line `frozenset` of double-quoted
strings reduced to its members so the writer's order decides nothing; a last site holding a lone
value is a fault rather than a comparison, `in` over two strings answering about substrings, and
what the close cost was a file, the scan and the registry both sitting within twenty lines of the
cap, so the value forms and the comparators moved to `scripts/values.py` and the overlay's half of
the registry to `scripts/overlaycouplings.py`, read back as one tuple; the body producing
`"image/gif"` and the allow-list dropping `"image/png"` each exit 1 where the previous commit's
scan exits 0 over the second of them, its reducer refusing a `frozenset` outright, the body's bind port 50151, whose only possible declaration site is
...
already tracks), a gRPC status code each language spells in its own casing, which a mention would
have to case-fold across to reach (triggered by a third caller of that table), and the roll's
duration restated as `0.3s` against a `MORPH_ROLL_MS` counting milliseconds, **struck 2026-08-09**
on the second of the two ways it named and with its own arithmetic corrected on the way: the sheet
spelled `0.3s` seven times and not thirty, six of them declarations, and `:root` now carries
`--roll: 300ms` for the two of those six that are genuinely the roll (the section share caps'
`max-height` and the thoughts marker's turn, each of which already said so in its own comment)
while the four that merely last as long keep their literal, a summon fade and three arrivals whose
tie to the roll would be a coupling nothing asked for; the unit-aware value form turned out to be
unnecessary rather than deferred, since a sheet that spells a duration once can spell it in the
constant's own unit, and what opened in its place is the spend side of a value-declaring property,
...
a first mistyped spend as its trigger, **struck 2026-08-11** ahead of both by the second of the two
ways it named, a mention rendering a NAME rather than a value, since the other way was a constant
written to be read by a gate: `Mention.name` plus a `{name}` placeholder makes the pair two mentions
of one entry, the declaration on `:root` and the `var()` that pays it, a name is carried exactly
when a template renders one, and a name pinned as a spend that no mention renders a value under is
refused; `var(--roll)` is counted at 2 and `var(--ease)` is a presence check, 52 transitions across
unrelated features riding that curve, which is the division the `[data-morphing` rules already had,
and what it leaves is one line narrower than the entry it came from, a mistyped spend of a
presence-checked name, whose honest close is a stylesheet-wide check that every `var()` names a
property something declares; and a fifth on 2026-08-10 by arrival rather than exchange,
...
rather than by anything that checks, **struck 2026-08-08** when the scan landed ahead of its
trigger and with a rule this bucket had wrong: comparing the bind defaults against `.gitignore`
...
occurrences are genuinely a set that has to move together, **struck 2026-08-09** on that trigger:
`Mention.occurrences` counts bounded matches instead of stopping at the first and pins an EXACT
number rather than a floor, since a floor passes on a far side that grew and then passes again
when it drops back, widening the gate by however much the tree drifted; the churn the entry feared
is held off by the field being opt in rather than by weakening the comparison, so two of fourteen
mentions are counted (`Message.tsx` at 2, whose two comparisons are one chip's class and its
accessible name, and a narrower `:not([{value}="0"])` at 2 for the two morph share caps) while the
three `[data-morphing` rules stay a presence check, three being the sum of two unrelated features
and so the arithmetic the entry declined; the rename applied everywhere but `Message.tsx`'s second
line exits 1 where the previous day's scan exits 0 ([repo-gates.md](repo-gates.md)).
...
its trigger being a sidecar that restarts under a running brain, and which **took a second
staleness on 2026-08-09** rather than letting it open an entry of its own, since the deadline
pairing that landed that day is read once at wiring time, so a sidecar coming back with a changed
environment leaves that check as stale as it leaves residency and the same generation counter
closes both. **It landed hours later the same day, both halves, in almost the shape its own text
proposed**, the one correction being that a counter was never usable, since a counter in a
restarted process starts again at the number the comparison exists to notice, so the identifier is
a random per process boot id. What its text did not price is that `converge_residency` cannot be
called speculatively, stopping and restarting every evictable tier being exactly what a
co-resident plan exists not to do to its peers, so a first observation is a seed and the boot
publish is what seeds it. It opened one entry in its place and the area count therefore held:
**reaching that reconciliation from a brain that cannot run a turn**, since it fires only inside a
swap, and a brain whose restore gave up refuses every acquire, so the state that most needs
reconciling is the one state that can never reach it, which is why the runbook's manual recovery
still ends by restarting the brain; its trigger is a second visit to that recovery path, or a
report a user rather than a reader of the code finds stale
...
handoff that has watched itself spill still promises co-residency next time**, the obvious second
actor for a watch whose only actor today is an operator reading a log, declined because it latches
a working feature off on evidence one handoff wide and because `ResidencyPlan` is a frozen value
with nowhere to keep the latch, so it is a residency-state question rather than a watch one; its
trigger is a deployment that spills repeatedly with nobody reading logs, or a second machine
adopting the co-residency flag from this repo's numbers. And **prefill as the second witness**,
which the same `timings` object already carries and which collapsed harder than decode did on the
co-residency run, declined because a prompt rate varies with prompt length far more than decode
does, making the floor a deployment must measure a harder number whose wrong setting produces false
collapses; its trigger is a spill decode misses, or deep answers short enough that decode rarely
clears the judgeable-sample floor.
...
The entry is sharpened rather than struck, in its own doc. **It closed 2026-08-09 on the other half
of its own trigger**, the recall default reproduced rather than cited, and both remaining halves
turned out to be one division of labour rather than one clever test. An arm is a container
configuration, so a restart is a deployment step and belongs in `just turn-cost`, which runs the
three blocks and recreates only the brain between them; that puts the arms in separate processes,
so each block writes a JSON sample and the interval is computed afterwards by `scripts/contrast.py`,
the first module in that tree that gates nothing, which pairs the blocks question by question and
bootstraps over a printed seed. The A/B/A control needed no separate answer: it is the recipe
running its outer blocks in one configuration and its middle block in another. Two of the entry's
claims did not survive the tree. Seeding was said to be settled by the fold run, whose corpus is
conversation history through `RedisSessionStore`, while this one is 41 memory notes through
`PgVectorMemoryStore` behind the CPU embedder, removed with `delete_scope`, so the discipline
carried over and the mechanism did not. And the harness could not have been written when it was
filed: no `CORTEX_MEMORY_*` knob reached the dockerized brain, which the memory runbook had been
documenting past since the override was written, and they are pass-through keys now.
...
read, and a read cannot settle a trigger whose subject is what a command does. Both live in
[repo-gates.md](repo-gates.md). **Standing test-order randomization** was measured with
`pytest-randomly` supplied for the run only, so no dependency entered either lockfile:
`uv run --with pytest-randomly pytest -p randomly --randomly-seed=N` at seeds 1, 2, 3, 20260810 and
987654321 over the whole brain workspace (2306 tests, 65 integration-marked deselected) and at the
same five over `scripts/` (400 tests). Ten runs, all green at the 100% coverage both suites already
demand, with `--collect-only` proving the order genuinely moved (seeds 2 and 3 share all 2306 node
ids and put none of them in the same place). The two failure kinds were separated before the runs
and neither appeared: no test failed on a sibling's leftover state, which is the trigger, and none
failed on the plugin reseeding `random`, which is an artifact the entry predicts and which has no
reachable consumer anyway, the only draw in the gated Python being a `random.Random(seed)` instance
inside `scripts/contrast.py` and the one unpredictable id coming from `secrets.token_hex`. Adopting
the plugin as a standing gate is **recommended against and left to the maintainer**, recorded in
the entry rather than taken. **The Tauri shell's `cargo clippy`** was run too, since the half of its
trigger about findings accumulating is checkable even though the half about CI is not:
`cargo clippy --all-targets -- -D warnings` in `body/app/src-tauri` exits 0 over the 978 lines in 12
files the shell now holds, where the entry records 881 in 11, and a planted `useless_format` makes
it exit 101 so the check is not vacuous. That run also corrected the entry's account of this host,
...
shared list or the first drift caught in the wild ([repo-gates.md](repo-gates.md)). **Still five on
2026-08-11**, when that trigger fired once and the entry stayed open: the next port to gain a shared
list was the overlay's `BrainBridge`, the first outside Python, thirteen named checks driven over
`FakeBridge` and `DemoBridge` by one parametrized driver, with `demoBridge.ts` and its script coming
out of the overlay's coverage `exclude` behind it and `tauriBridge.ts` staying in it because every
method of it is an `invoke` call, so a shared driver over it would fake the IPC and measure the
fake. The list reddened on three disagreements before either implementation was changed to suit it, and
on a fourth thing its turn-handle check asked about, a capture the demo announced inside the
`converse` call where the real bridge's channel can only deliver after it; it was then shown to
fail three more times on breaks planted in the demo bridge and restored. The
entry stays open on the four Python ports, the streaming half of `InferenceBackend`, and every Rust
row, where the fakes themselves are still written twice
([repo-gates.md](repo-gates.md)). **Still five later that same day**, when the first of the
four Python ports gained its list: `Embedder`, four checks driven over `HashEmbedder` and over
`LlamaCppEmbedder` on a `MockTransport`. That one found no behavioural disagreement between its two
implementations, which is what a port one method wide can honestly yield, and it is recorded rather
than left as a silence; what it did find is a fake with no way to raise the single error the port
documents, so nothing in the core could exercise a remember or a recall against a dead embedding
server. The fake gained a scripted failure, the list was proven able to fail once per arm and once
per side of that new knob, and the entry stays open on `ToolRegistry`, `BodyGateway`, `Confirmer`,
the streaming half of `InferenceBackend`, and every Rust row ([repo-gates.md](repo-gates.md)). **Still
five for the second of them**, `ToolRegistry`, which is the one that paid. Its six checks run over
three implementations, the core's fake and both MCP registries, and the fake could express neither
the port's central case, a tool that ran and reported an error, nor its world, a tool set it copied
at construction and no way to be unreachable. One divergence was decided against the port's own
wording rather than against either implementation, since only a registry that knows its whole set
can raise `ToolNotFoundError` for an unknown name and a remote one can only relay what its server
says. Four breaks reddened only the arms that can carry them, one of them a listing cache that
reddens the translating MCP arm and not the reconnecting one, which is why both are in the list
([repo-gates.md](repo-gates.md)). **Still five for the third**, `BodyGateway`, where the
finding runs the dangerous way: its fake handed back a capture the real adapter would have
refused, a non-zero size bound being a bound on the reply that only the adapter re-verified, so a
core test could watch a turn accept a picture production would have thrown away. A fake more
permissive than its adapter hides defects rather than inventing them, and the rule turned out to
be domain logic rather than wire translation, so it moved into the core and both implementations
call it now. Ten checks over the fake and over the adapter against a `BodyService` on loopback,
proven able to fail four times, twice per side ([repo-gates.md](repo-gates.md)). **Still five for the fourth and last of the
Python four**, `Confirmer`, whose five checks run over the core's fake and over `SeamConfirmer`
with a scripted overlay wired into its `emit`. No behavioural disagreement came out of them, which
for a port whose whole contract is that only an explicit yes is `True` is the answer worth having,
and the one thing that did was a fake whose answer was fixed at construction where a person is not
a constant. One break deliberately did not redden: a `resolve` answering whichever ask is pending
rather than the one it was given leaves every shared check green, because only one ask is ever
outstanding through the port, which is the division of labour rather than a hole, the adapter's
own suite holding the id matching. **The entry stays open with its Python half done**: what is
left is the streaming half of `InferenceBackend`, a design question rather than a transcription,
and every Rust row ([repo-gates.md](repo-gates.md)).

**One entry arrived in this bucket out of that work rather than beside it**, which is what a
contract list is supposed to do: writing the `Embedder` checks established that both
implementations raise `EmbedderError` and nothing else, and then that **nothing in the brain
catches it**, nor the store's error either. A stopped embedding server or an unreachable Postgres
therefore fails the turn instead of costing it its recalled notes, which is the opposite bias from
every other optional capability here, where a dead tool sidecar is served around, a body that will
not answer becomes a recoverable result, and a subagent that cannot be admitted degrades. The
remedy is a decision rather than a line, about where the catch belongs and whether a turn that
silently forgets should say so, so it is filed rather than taken ([memory.md](memory.md)).
**It was taken the same day, hours later**, once the decision it was waiting on was made rather
than deferred, and its account of the code was re-derived by running a turn against a failing
embedder before any of it was believed. Both halves degrade and for opposite reasons: the read
because a turn has an answer without its notes, the write because the reply has already streamed
and the exchange is already in the conversation, so raising loses the memory anyway and takes a
read turn with it. Only the two errors the ports declare degrade, and the report is unconditional
on the module logger rather than a line on the opt-in recall trail, with the user told once, about
the read, on the channel a fold narrates itself on. The two it opened in its place are both
residue of the close: the adapter wraps a malformed row into the same error an unreachable server
raises, so a data defect now degrades wearing an outage's clothes, and the `MemoryStore` shared
check list still has no backend-failure check where the `Embedder` list has one, which is why the
store's twin gained a `fail_with` of its own instead ([memory.md](memory.md)).
**The first of those two closed the same day as well**, hours after it opened and ahead of its own
trigger, no malformed row having been met: `MemoryDataError` subclasses `MemoryStoreError` so
every existing catch is unchanged, the adapter raises it from its three decoding catches and from
nothing else, and the read names it ahead of the degrading catch and re-raises rather than
degrading with a louder log. The line is whether the condition heals with nobody touching the
deployment, which is why an outage degrades and a row that will not decode does not, and the
user-facing status stays exactly as it landed, a turn that is not going to answer having no
business saying it is answering without earlier notes. The adapter turned out to be able to tell
the two apart where it wraps rather than having to guess, the conditions arriving as disjoint
exception types, and the shared list took only the half both implementations can answer, that a
gone backend must not arrive as the subclass. One opened in its place, the delete cascade's seam
mapping, which calls the new subclass `UNAVAILABLE` like everything else and so offers a retry for
the one condition no retry improves ([memory.md](memory.md)).
**The second of those two closed the same day**, an hour after it opened and ahead of a trigger
neither arm of which fired, because a degradation resting on an unheld guarantee is the gate that
cannot fail: the shared list now drives eleven checks over a `MemoryStoreUnderTest` pair, and the
new one requires `add`, `search`, `count_candidates` and `delete_scope` to answer
`MemoryStoreError` and not the driver's exception with the backend taken away. The live arm is
broken by the adapter's own `aclose`, so the pool really closes and the check reads
`PgVectorMemoryStore`'s translation of asyncpg's `InterfaceError` rather than a stand-in for it.
Neither implementation leaked, and the check was reddened on each arm alone and restored before
being trusted ([memory.md](memory.md)).
