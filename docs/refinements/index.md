...
**161 open, 500 closed, 661 in total.**
...
- **[R-422](tasks/422-a-newly-ignored-tree-reaches-the-list-by-hand.md)** A tree that joins .gitignore reaches the shared skip list only by hand (repo-gates). Reopens when: a file one of the three walks reads appears inside a directory git ignores that `SKIPPED_DIRS` does not name: a `.py`, `.rs`, `.ts` or `.tsx` for the cap, a `.md` for the anchor scan, a `docker-compose*` or `compose*` `.yml` or `.yaml` for the compose walk. The bare disagreement is not the trigger, because the two collections already disagree about five directories that exist today. Its claim was re-derived from the code on 2026-09-14.
