...
| [untrusted-content.md](untrusted-content.md) | Taint boundary, output guardrail, subagent model safety (ADR-0013/0015/0017/0019/0028), a quoted injection replayed by the plain history window, obeyed 2 of 10 on a bare turn and 0 of 10 behind either standing rule, the plain one landed for the tool-less turn (ADR-0013/0038), and, as of 2026-08-08, two guardrail bypasses fewer with the **count deliberately unmoved**, since neither was ever counted: pricing the encodings tail against the shipped module found a URL spelled in the fullwidth and CJK twins of its own punctuation, a CJK-dotted host leaking past the default policy (the resolver reads `。` as a dot, `encodings.idna.dots`, and NFKC maps `｡` onto it rather than to a dot) and a fullwidth scheme separator (`https：//`) anchoring no match at all and so invisible to both policies, which is the seventh addendum's severe shape again. The tail that was being priced stays open and now carries its table: source-code escapes and a bracket-less entity colon still fold or anchor nothing, deferred on the line the fix drew, that a renderer or the resolver undoes every obfuscation closed so far while a source-code escape asks the reader to decode by hand (ADR-0015) | 12 |
...
it ([untrusted-content.md](untrusted-content.md)); the GBNF alternative, the
