...
**96 open, 193 closed, 289 in total.**
...
- **[R-287](tasks/287-rust-tests-run-in-one-fixed-order.md)** The Rust suite runs in one fixed order (repo-gates). Reopens when: a Rust test that passes alone and fails inside `cargo test`, or any order-dependent flake in the body workspace.
- **[R-288](tasks/288-nothing-schedules-the-shuffle-sweep.md)** Nothing schedules the shuffle sweep (repo-gates). Reopens when: an order dependency found by a hand-run sweep that the standing seed had already run past, which is the case the standing gate provably cannot draw.
...
- [R-287](tasks/287-rust-tests-run-in-one-fixed-order.md) The Rust suite runs in one fixed order. open, fix when it bites.
- [R-288](tasks/288-nothing-schedules-the-shuffle-sweep.md) Nothing schedules the shuffle sweep. open, fix when it bites.
