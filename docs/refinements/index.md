...
**177 open, 433 closed, 610 in total.**
...
- **[R-600](tasks/600-the-plain-cells-application-rate-is-measured-and-its-direction-is-not.md)** The plain cell's application rate is measured and its direction is not (vision).
- **[R-601](tasks/601-the-plain-framed-cell-at-the-third-frame-reads-1-of-5-and-4-of-5.md)** The plain framed cell at the third frame reads 1 of 5 and 4 of 5 (vision).
- **[R-602](tasks/602-the-plain-controls-fall-at-the-third-frame-is-read-off-twelve-draws.md)** The plain control's fall at the third frame is read off twelve draws (vision).
- **[R-603](tasks/603-the-engine-budgets-deep-row-voids-on-draws-that-think-to-the-cap.md)** The engine budget's deep row voids on draws that think to the cap (vision).
...
- **[R-375](tasks/375-a-flagged-name-shut-is-dropped-as-if-missing.md)** A flagged name that is merely shut is dropped as if no mailbox had it (email-confirmer). Reopens when: a server this repo can reach listing a name that is both flagged unselectable and refused in words other than the ones that prove a folder missing, read off a LIST past the port rather than off the offered list
...
- **[R-557](tasks/557-the-engine-image-names-are-typed-in-five-places.md)** The engine image names are typed in five places (inference). Reopens when: a compose file or the model-host Dockerfile naming a different engine image or tag than the one it names today, which is the retag the ADR-0005 engine-tag addendum says is one `docker compose pull` away; or a second harness starting its own server from a typed image name.
- **[R-558](tasks/558-thinking-follows-the-tiers-name-and-not-its-shipped-budget.md)** Thinking follows the tier's name and not its shipped budget (inference). Reopens when: a deployment that starts the cortex or the deep tier with `CORTEX_REASONING_BUDGET` or `CORTEX_REASONING_BUDGET_BRAIN` at zero and wants the injection harness to draw that tier's rows as it runs them.
- **[R-559](tasks/559-the-cpu-row-carries-the-cpu-quota-and-not-the-memory-cap.md)** The CPU row carries the CPU quota and not the memory cap (inference). Reopens when: a subagent candidate whose resident size on the CPU approaches the override's `CORTEX_SUBAGENTS_MEM_BUDGET_GB`, or a CPU row whose reading a reader wants to attribute to the container's memory rather than to the model.
...
- [R-557](tasks/557-the-engine-image-names-are-typed-in-five-places.md) The engine image names are typed in five places. open, fix when it bites.
...
- [R-559](tasks/559-the-cpu-row-carries-the-cpu-quota-and-not-the-memory-cap.md) The CPU row carries the CPU quota and not the memory cap. open, fix when it bites.
...
- [R-598](tasks/598-the-leaks-denominator-is-53-in-one-place-and-58-in-three.md) The leak's denominator is 53 in the runbook and 58 in the three other places that publish it. open, actionable.
...
- [R-299](tasks/299-prose-cites-an-engine-build-nothing-pins.md) Prose cites an engine build that nothing pins. open, actionable.
...
- [R-600](tasks/600-the-plain-cells-application-rate-is-measured-and-its-direction-is-not.md) The plain cell's application rate is measured and its direction is not. open, actionable.
- [R-601](tasks/601-the-plain-framed-cell-at-the-third-frame-reads-1-of-5-and-4-of-5.md) The plain framed cell at the third frame reads 1 of 5 and 4 of 5. open, actionable.
- [R-602](tasks/602-the-plain-controls-fall-at-the-third-frame-is-read-off-twelve-draws.md) The plain control's fall at the third frame is read off twelve draws. open, actionable.
- [R-603](tasks/603-the-engine-budgets-deep-row-voids-on-draws-that-think-to-the-cap.md) The engine budget's deep row voids on draws that think to the cap. open, actionable.
