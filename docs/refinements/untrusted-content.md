...
**Open items:** the screening subagent, Windows-native validation of the confirm card, whitespace-split hosts, the full UTS-39 confusables set, mixed/other encodings, footer/boilerplate heuristics, a structured redaction event for the overlay, a raw GBNF grammar alternative, a per-task caller-supplied schema, a sidecar-declared sender/URI source, provenance across the stores, a fence-without-block recall mode, per-provenance eviction, per-remote-tool trust/gating overrides, taint persistence across a mid-turn swap, the brain-tier injection-harness run
