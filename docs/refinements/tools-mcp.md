...
cost-aware batch cap, fair-share policy across a batch (the sidecar session cache/pool left this
list 2026-08-08, declined on a measured 17.8 ms open, the sidecar's own per-request process spawn
having been the whole of the cost it was blamed for)
...
- **The session pool is declined 2026-08-08, measured ([ADR-0009 handshake
  addendum](../adr/ADR-0009-tools-mcp.md)).** The entry above priced the per-call open with an
