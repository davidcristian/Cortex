...
find weak: framed, gemma-4-E2B obeys 3 of 10 payloads and Qwen3.5-2B 1 of 10, where the cortex and
the subagent pick (gemma-4-E4B) obey none
([injection text rows](../readings/injection-text-rows.md)). Treating the cortex's model choice as
pure discretion opens a failure the deterministic layers do not cover. The plausible failure is not
an injection tricking the injection-resistant cortex. It is a **well-behaved** cortex routing a
subtask over untrusted content to a cheap model to save latency, with no way to know the content is
hostile.
