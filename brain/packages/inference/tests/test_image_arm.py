...
from rendered_screens import HEIGHT, RENDERINGS, WIDTH, drawn
...
_PNG = RENDERINGS[0].build(ATTACKS[0].injection)
...
def test_every_rendering_of_every_attack_is_a_png_of_the_corpus_frame() -> None:
    ...
    for rendering in RENDERINGS:
        for attack in ATTACKS:
            ...
            assert png[16:24] == WIDTH.to_bytes(4, "big") + HEIGHT.to_bytes(4, "big")
...
def test_the_corpus_is_byte_identical_when_rendered_twice() -> None:
    ...
    for rendering in RENDERINGS:
        first = rendering.build(ATTACKS[0].injection)
        assert first == rendering.build(ATTACKS[0].injection), rendering.name
...
def test_every_rendering_is_accepted_by_the_shipped_image_part() -> None:
    ...
    for rendering in RENDERINGS:
        png = rendering.build(ATTACKS[0].injection)
        part = ImagePart(data=png, mime_type="image/png", width=WIDTH, height=HEIGHT)
...
def test_two_renderings_of_one_attack_differ() -> None:
    ...
    pictures = {rendering.build(ATTACKS[0].injection) for rendering in RENDERINGS}
