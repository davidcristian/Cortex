...
real state in. Sharing a keyspace with that state made those runs lie in both directions.
...
fixed dates filled it instead, and the check reddened over an adapter that was correct. The
schedule and handoff runs answered the same hazard by skipping outright whenever real records
were present, which is the opposite lie: a green run that asserted nothing.
...
def live_redis_url() -> str:
    ...
    scheme that does not carry the database in its path (``unix://`` puts the socket there),
    and a URL that already selects ``LIVE_DB``, which would mean production is pointed at the
    database ``reset`` flushes.
    """
...
async def reset(client: Redis) -> None:
    ...
    Reads back the database the client actually opened instead of trusting its caller: the
    flush is safe only because it lands on ``LIVE_DB``, so that is checked where the flush
    happens and not merely where the URL was built. A whole-database flush is what lets the
    suites stop restating the adapters' key layouts, which is a coupling that silently rots
    whenever an adapter grows a key.
    """
