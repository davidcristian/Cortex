"""LoggingRecallSink logs each recall; ``recall_fields`` names the fields both trails keep."""
...
def recall_fields(audit: RecallAudit) -> dict[str, object]:
    """The pool, what it was drawn from, the basis, the hits and the drops, never the texts."""
    return {
...
class LoggingRecallSink:
    ...
    async def record(self, audit: RecallAudit) -> None:
        """Log one recall as the fields ``recall_fields`` names."""
        _logger.info("memory.recall", extra=recall_fields(audit))
