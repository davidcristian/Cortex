...
A bound this deployment sets is usually validated where it is declared: ``ToolsConfig`` fails on a
price outside the dispatch budget, ``SubagentsConfig`` fails on a run deadline that does not
outlast its own stall ceiling, and each is a relation between fields one class can see. A few
relations hold between numbers two settings classes declare, so neither class can express one, and
the comparison belongs at the composition root that reads both.
...
def delegated_call_bounds(tools: ToolsConfig) -> int:
    ...
    The bound is spent per walk rather than per dispatch, which is what makes a bare comparison of
    the two numbers wrong rather than merely loose. Every one of these reaches
    ``BoundedToolRegistry`` separately and every one of them can find a wedged sidecar:

    - the run's advertisement, one walk, which ``stream_tool_loop`` makes once before its rounds;
    - the gated strip, one walk, which ``UngatedToolRegistry.invoke`` makes live on every delegated
      dispatch, deliberately uncached so a re-flagged tool fails closed;
    ...
    A walk costs one bound per wedged sidecar it lists, since ``SkipUnavailableToolRegistry``
    catches each overrun and carries on, so the ceiling on a walk is the endpoint count and the
    ceiling here is that count times the walks plus the call. It is deliberately an upper bound:
    the ``fail`` policy aborts a walk at the first overrun, and one wedged sidecar among healthy
    ones costs one bound a walk rather than several. Measured against the real composition at 60 s
    and one endpoint, a delegated dispatch spends the bound twice and the advertisement before it
    once more, which is this function's 3.

    It deliberately does not cover a whole run. A run makes many dispatches over many rounds, so
    this is what the first wedged dispatch costs and never what a whole run can. Bounding the run's
    worst case would mean multiplying by ``MAX_TOOL_DISPATCHES`` as well, which the shipped pair
    does not clear and which would fail deployments that work: the failure this exists to prevent
...
def check_tool_call_deadline(subagents: SubagentsConfig, tools: ToolsConfig) -> SubagentsConfig:
    ...
    ``SubagentsConfig`` declares. Those three nest by the scope each bounds: one gap with no output
    inside one whole run inside the queue for a run. This one is the innermost's sibling rather
    than its child, bounding one tool dispatch where the stall ceiling bounds one silent read, and
    both of them sit inside the run. A run failing to outlast the stall ceiling is caught at boot
    already; this fourth bound was ordered against none of them.

    The ordering is measured rather than reasoned. With the pair the right way round, a wedged
    sidecar loses one call at the bound, the ``ToolError`` becomes the ``is_error`` result the loop
    recovers from, and the subtask still answers. With the pair inverted, the run's own deadline
    fires first and the whole delegated run comes back ``TRUNCATED`` with no text at all, under a
    detail saying the subtask "was still generating" and should be narrowed before it is delegated
    again. That sends the reader to the model and the instruction when the cause was a sidecar two
    knobs away, and it skips the CPU re-run as well, a truncation being deliberately never
    re-placed. So a pair nothing checks removes, for delegated work, the bound that exists to make
    a wedge survivable.

    This raises rather than clamping or logging. A clamp would retune a number the operator typed,
    ...
    diagnosis, and a boot line nobody greps goes unread as easily as the knob it describes. The one
    place a warning is right is where ``check_control_deadline`` puts one, against a far side this
    process cannot query, and both numbers here are this process's own env, always readable.

    What is compared is the whole dispatch rather than a single call bound. A single call bound
    under the run bound is not enough, because one dispatch spends that bound several times over:
    ...
    spends 1400 s inside a run allowed 900, which is verbatim the outcome below. That pair fails at
    boot instead.

    The comparison is strictly under, the ``ControlBounds.clears`` rule: a dispatch allowed the
    whole of the run would make which bound fires a race, and the expensive side of that race is
    the one reporting a wedge as a runaway. It runs only when both capabilities are on, the shape
    of ``check_control_deadline`` returning early for a deployment that never escalates: without
    ...
    A passing check promises that the first wedged dispatch of a delegated run reaches the loop as
    a ``ToolError`` rather than being cut mid call by the run's own deadline, which is the
    difference between a subtask that reports a broken sidecar and a run reported ``TRUNCATED`` for
    reasons that point at the model. It promises nothing about the run finishing: a run may
    dispatch many times, and the arithmetic that would cover all of them is in
    ``delegated_call_bounds``, along with why it is not the one used here.

    Checked at the env read, before anything at all is built, so a failure here releases nothing.
    That is earlier than the control deadline's own check, which has a runtime to close first
    because querying the far side is what tells it there is a fault.
    """
