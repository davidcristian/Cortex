...
The two deadline defaults are imported rather than restated. ``cortex_body_client`` owns the
calls, so it owns how long they may take; a settings module that declared its own ``5.0`` would be
a second default that only looks like the first. ``config.py`` does the same with the session
adapter's ``DEFAULT_REDIS_URL``.
"""
...
# The edge the brain asks for when nothing overrides it, declared here rather than written inline
# in the ``Field(...)`` below because it is not only this module's: the compose stack ships it as a
# substitution default and the vision runbook quotes it as the number a deployment is running, so
...
class BodyConfig(BaseSettings):
    ...
    Two capture bounds travel with a request (ADR-0029). ``capture_max_edge`` and
    ``max_image_bytes`` are what the brain asks the body for and what it holds the reply to: the
    body clamps both and an older body ignores both, so they are re-verified on receipt.
    ``max_image_bytes`` defaults to the same 6 MiB as the body's own ceiling, which is the point of
    sending it rather than relying on two constants to stay equal.
    ...
    paying for is the model's per-image token budget, which the body has no access to; a body
    asked for nothing keeps answering at its own conservative 1600, where a worst-case
    incompressible screen still encodes inside the byte ceiling. ``0`` still means "the body's own
    default", so a deployment can hand the choice back.
    ...
    Folding both onto one number would either cut a capture short or give a volume read ten
    seconds it can never use.

    All four are bounded here so a misconfiguration fails at boot, the way the model host's ports
    and context sizes do. Both capture bounds travel in uint32 proto fields, so a negative or
    over-wide value is a request that cannot be built at all, and left unbounded they would turn
    every capture in that deployment into a turn-killing exception rather than a failure at
    startup. ``max_image_bytes`` may only tighten the domain ceiling (the body clamps to its own
    anyway, so a looser number is a bound nothing would honour), and ``capture_max_edge`` may not
    exceed the largest edge an ``ImagePart`` would accept, which the reply is checked against too.
    A deadline is positive because a zero or negative one is a call that can never succeed.
    """
