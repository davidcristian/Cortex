...
def build_builtin_tools(
    ...
    *,
    ...
    Advertising it otherwise would offer the model a tool that could only fail, the same rule
    that keeps the volume pair out without a body and task scheduling out without delegation.

    `vision` (ADR-0029) is that same rule for `capture_screen`: it needs a body to take the
    picture and a model that can read it, so it is advertised only when the composition root
...
def build_cortex_tools(
    ...
    *,
    ...
    None when nothing is enabled, which leaves the tool-less turn path unchanged. `builtins`
    arrives pre-assembled from `build_builtin_tools`, as one sequence rather than one parameter
    per capability. The `CompositeToolRegistry` gives the built-in tools precedence and
    ...
    declaration, `salience` (`CORTEX_TOOLS_SALIENCE`), is what rejects a call that loop has
    already made (ADR-0009 salience addendum). This is the dispatcher the default
    `spawn_subagents` price applies to: built-ins are cortex-only, so the subagent and ticker
    dispatchers never advertise it.
    ...
    `capture_screen` follow it: hidden from the advertisement and rejected at the call while the
