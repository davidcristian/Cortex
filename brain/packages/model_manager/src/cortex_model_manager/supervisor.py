"""One ``llama-server`` child per logical model: start, stop, and report its state (ADR-0030 d3).

The whole behaviour behind the ``ModelHost`` port's three verbs, over the ``ChildProcesses`` and
``HealthProbe`` seams so all of it is gated without spawning anything. Each guarantee the port
makes comes from a rule here:
...
- Start and stop are idempotent, because a swap re-issues either without checking first
  ...
  no-op, and a start whose spawn fails adds nothing. A failed spawn also removes nothing: a tier
  whose previous child had died goes on reporting that child's exit code rather than being erased
  by the replacement that could not be started, since the spawn's own failure is what the ``start``
  raises and the exit code is what the runbook reads.
- ``start`` returns long before the model is ready. A start is a spawn and nothing more, so the
  swap's health gate is the only thing that determines readiness. Blocking here would bound a
  minutes-long load by an HTTP client's timeout instead of by the plan's bound.
- ``stop`` does not return until the child is dead and reaped, so its VRAM is free before the
  caller starts the next model. ``swap_in`` stops the cortex and starts the deep model with
  nothing in between, so a still-dying cortex holding ~11 GB would CUDA-OOM the load.
- ``status`` reads the process before it reads the probe. Measured: a child that failed to bind
  dies in ~0.24 s with exit code 1 while the previous model keeps answering 200 on that port, so a
  status that proxied ``/health`` alone would report the dead model READY and leave the old weights
  resident, breaking the swap with nothing reporting it. The exit code takes precedence.
- One lock per logical model serializes its three verbs, because a stop racing a start is the race
  that produces the bind failure above.
"""
...
# How long a child gets to exit on SIGTERM before it is killed. Measured on the dev GPU, where the
# two cases are far apart: an idle llama-server exits 0 in 0.14 s to 0.40 s, while one with a
# request in flight does not honour SIGTERM at all (it logs "cleaning up before exit" and then
# blocks on the generation, the shipped tiers running --parallel 1), so it is killed and the whole
# grace is paid: 10.09 s and 10.90 s end to end on the stand-ins. The number is therefore the real
# cost of evicting a busy tier rather than slack around the idle case, and tuning it down on the
# strength of the idle number would SIGKILL a model mid answer. A deployment running a differently
# sized model should re-measure it (docs/runbooks/model-swap.md).
...
class ModelSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        # identifies the child table below: a brain that reads a value it has not seen before can
        # tell that its record of what is resident was formed against a table that no longer
        # exists. Random rather than counted, because a counter in a process that restarted begins
        # again at exactly the number a reader compares against to detect the restart.
    ...
    def boot_id(self) -> str:
        ...
        whose in-memory child table the value identifies, and the route that publishes it is a
        serializer with no state of its own. Nothing in this process reads it back; its only
        consumer is a brain checking whether its record of the GPU was formed against this daemon
        or against the one that ran before it.
        """
    ...
    async def stop(self, model: str) -> None:
        ...
        async with self._locks[model]:
            ...
            if child.returncode is None:
                # Deliberately before the delete: a child that has not exited still holds VRAM, so
                # it must keep being reported rather than be recorded as STOPPED. The caller's
                # retry then tries again on the same process.
    ...
    async def _end(self, model: str, child: ChildProcess) -> None:
        ...
        if await self._reaped(child, self._bounds.reap_timeout_s):
            return
        # Raised and not also logged, unlike the other failures of this shape in the repo. Both
        # callers of ``stop`` log what they catch, the API with this sentence on its ``error``
        # field and the shutdown sweep with the traceback under its own, so a line here would print
        # the same event twice and print its numbers twice within the one line, once in the prose
        # an exception's text has to carry and once in the fields beside it. The API's line
        # supplies the level, which follows its status code.
        msg = (
