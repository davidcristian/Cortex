...
Four things about the shape of this seam:

- It shells out to ``nvidia-smi`` rather than binding a library. The binary is injected into the
  container by the NVIDIA container toolkit alongside the driver, so it is present wherever a GPU
  is reserved and absent wherever one is not, which is the condition this seam has to report. A
  Python NVML binding would add a dependency to the image for one number and would still be a
  wrapper around the same driver library.
- Every failure returns "no reading" rather than raising. A missing binary, a non-zero exit, and a
  body that does not parse all mean the daemon cannot see a card, which is a normal condition (a
  CPU-only stack, a container with no device reserved) and not an error the control API should
  turn into a 503. Fail-closed lives on the brain's side, where a swap that required a fit fails
  when the answer is ``None``.
- More than one visible device also returns "no reading". The stack reserves one GPU (ADR-0012)
  and everything downstream compares a single free figure against a single model's cost. Picking a
  row out of several would be a guess about placement that nothing in this repo makes, so this
  seam reports no reading instead, logged where an operator will see it.
- The call is bounded. It runs inside a swap step on the brain's side, under one control deadline,
  so a hung ``nvidia-smi`` must not hold the request open. The bound is the readiness probe's,
  both being control-plane reads that may never outlive a swap step.
"""
...
class NoDeviceMemory:
    ...
    This is what a CPU-only deployment and the test suites are wired with. It exists so the "no
    reading" path is a real object rather than a ``None`` collaborator every caller has to branch
    on.
    """
