...
def test_apply_edit_leaves_unset_fields_and_can_clear_recurrence() -> None:
    ...
    assert unchanged.text == "keep"
    assert unchanged.every == timedelta(hours=1)
    assert unchanged.tainted is True
    cleared = apply_edit(item, ScheduleEdit(set_every=True))
    assert cleared.every is None
...
def test_apply_snooze_keeps_a_recurring_grid_on_the_pre_snooze_due() -> None:
    ...
    assert snoozed.due_at == _NOW + timedelta(minutes=20)
