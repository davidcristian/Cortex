...
calls reach the context is asserted here on its own. What the loop does with the plan (rejecting
...
def _calls(count: int) -> list[ToolCall]:
    """``count`` distinct calls, in the shape both the pool and salience admit."""
...
def test_a_round_of_exactly_the_cap_is_still_untouched() -> None:
    ...
    # dispatch, so a round of exactly that many dispatches all of them and appends no notice.
...
def test_one_call_past_the_cap_keeps_it_as_the_overflow_slot() -> None:
    # The other side of the boundary: the extra call keeps its place as the slot that carries
    # the rejection notice, so a round of cap+1 dispatches the cap and rejects exactly one.
...
def test_a_runaway_round_is_cut_to_the_cap_plus_one_slot() -> None:
    ...
    # whatever the pool did with them, because a rejection is appended too. The plan is what
    # bounds the round's footprint: cap dispatches plus one notice, and nothing else.
...
def test_an_empty_round_plans_nothing() -> None:
    # The loop breaks out before planning a tool-free round, but the plan must not create an
    # overflow slot for one, and an implementation reaching for `calls[-1]` to build that slot
    # would raise on an empty round.
    plan = plan_round([])
    assert plan.calls == ()
    assert plan.overflowed is False
...
def test_the_assistant_message_records_the_calls_it_is_given() -> None:
    ...
    # round never answers is the malformed conversation the loop's rejection notices prevent.
    calls = _calls(2)
...
def test_an_untrusted_result_is_fenced_before_it_re_enters_the_context() -> None:
    ...
    # the round cap must not have unfenced it.
