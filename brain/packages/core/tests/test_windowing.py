...
async def test_oldest_turns_drop_first_and_whole() -> None:
    ...
    # t3 (2 chars) and t2 (4) fit a budget of 6; t1 (8) overflows and drops whole.
...
async def test_a_turn_is_never_split_mid_exchange() -> None:
    ...
    # A budget of 5 fits t2 (4) and one of t1's two messages (3 each), so t1 drops whole: the
    # model never sees an assistant reply without the user message it answered.
