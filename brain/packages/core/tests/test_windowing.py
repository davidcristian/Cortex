"""Behavior of the session-history window policies (ADR-0014)."""
...
def test_empty_history_stays_empty() -> None:
    assert CharBudgetHistoryWindow(10).select(()) == ()


def test_under_budget_history_is_untouched() -> None:
    history = [*_turn("t1", "aa", "bb"), *_turn("t2", "cc", "dd")]
    assert CharBudgetHistoryWindow(8).select(history) == tuple(history)


def test_oldest_turns_drop_first_and_whole() -> None:
    ...
    selected = CharBudgetHistoryWindow(6).select(history)
    assert [m.turn_id for m in selected] == ["t2", "t2", "t3", "t3"]


def test_a_turn_is_never_split_mid_exchange() -> None:
    ...
    selected = CharBudgetHistoryWindow(5).select(history)
    assert [m.turn_id for m in selected] == ["t2", "t2"]


def test_newest_turn_is_kept_even_when_oversized() -> None:
    history = [*_turn("t1", "aa", "bb"), *_turn("t2", "x" * 100, "y" * 100)]
    selected = CharBudgetHistoryWindow(10).select(history)
    assert [m.turn_id for m in selected] == ["t2", "t2"]


def test_current_turn_user_message_survives_alone() -> None:
    ...
    selected = CharBudgetHistoryWindow(4).select(history)
    assert [m.text for m in selected] == ["the current question"]


def test_selection_is_a_contiguous_tail_not_a_sieve() -> None:
