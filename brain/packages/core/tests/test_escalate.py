...
async def test_a_turn_that_looked_at_the_screen_cannot_escalate() -> None:
    """Pixels are turn-local: no store persists them and the handoff record refuses them, so a swap
    would hand the deep model a tool message saying a picture is attached with no picture
    attached.
    """
