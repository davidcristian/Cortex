...
costs scheduling rather than time.

Mutations proving these tests can fail (each was applied on its own, then restored):
- returning READY unconditionally from the gate's timeout branch fails
  ``test_the_gate_reports_the_last_state_when_the_bound_elapses``;
- computing the gate's deadline inside the poll loop fails
...
- dropping the ``fail_once`` pop (making it permanent) fails
  ``test_a_scripted_failure_can_be_armed_for_one_call_only``;
- applying the fake's effect after its pause rather than before fails
