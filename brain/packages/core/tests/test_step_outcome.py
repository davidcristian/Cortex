...
# The body sends this when the host kill switch is off and when the overlay could not exclude
# itself, so by the time the brain sees them the two failures are one string.
...
class _Clock:
    """A clock fixed at one instant; nothing here checks a time."""
...
class _CallThenAnswer:
    """One round asking for ``name``, then a final answer."""
...
class _CaptureRegistry:
    """A ToolRegistry over the real built-in tool, so the image comes back in a real result."""
...
async def test_every_announced_dispatch_is_settled_exactly_once() -> None:
    ...
    class _ThreeRounds:
        """Three rounds, each asking for a different path, which the salience policy admits."""
