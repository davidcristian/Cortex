...
The deep phase watches decode cadence because a handoff that overcommits the card **succeeds**:
...
is precisely the deployment the automatic co-residency latch was declined in favour of.

So the verdict rides the same surface a missing peer already rides: a **serving** residency report
gains a detail, ``Health`` prefers that detail over its version string, and the overlay renders it
after "Brain ready" with no overlay change at all. It is composed at **read time**, in
...
**The standing rule, which is the whole design question.** A spill is a fact about one handoff and
the record it rides is a fact about now, so a note that never cleared would be a second way to be
...
- **Otherwise it lapses on its own, after ``DEFAULT_SPILL_DWELL_S``.** Escalation is rare on a
  ...
  afternoon is not still being described by a verdict about the morning.
...
# What ``Health`` says while the last handoff is believed to have spilled. It rides a **serving**
# report, so the overlay renders it after "Brain ready": turns work and delegation works, and what
...
class HandoffPace:
    ...
    def note_on(self, report: ResidencyReport) -> ResidencyReport:
        ...
        verdict on the *previous* handoff would be read as a verdict on that one.
