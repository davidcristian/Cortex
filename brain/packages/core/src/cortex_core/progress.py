...
# delegated step is announced here and never settled. Three reasons, none of them about cost: the
# one consumer of an outcome reads a single tool name (``capture_screen``) and that tool is a
# built-in the subagent dispatcher is never handed, so every forwarded outcome would be discarded
# by construction rather than by accident; a subagent's tools are the ungated subset, so no
# delegated step carries a consent decision for a surface to report; and ``emit`` below drops on a
# saturated buffer while the turn's own events block for a credit, so a pairing promised across
# this channel could not be relied on anyway. The widening is two lines and waits for a surface
# that renders how a delegated step ended.
