...
Every value is inert data and never authority. A source string can be attacker-chosen (an email
sender, a fetched locator), so it is bounded and sanitized here, in the pure core, at the moment a
``Provenance`` is constructed: no constructor can hold an unsanitized value, so no adapter can
...
display channel the reply-side guardrail never inspects, which is the same surface ``ToolStep``
already keeps closed (``tool_loop``).
...
class SourceKind(Enum):
    ...
    def attested(self) -> bool:
        ...
        is the brain's own (a registry-advertised name, an id it minted) and reads as a label,
        while a claimed one is attacker-choosable and reads as a quotation, however inert it has
        been made.
        """
...
def _inert(raw: str) -> str:
    ...
    newline is a control character too, so dropping it outright would join the words it
...
class Provenance:
    ...
    compares, so no kind is case-folded or otherwise rewritten into another's namespace.
    """
