...
accounting is told while it happens. Both are pure policy the residency scope calls, and this one
is deliberately tiny: a placer is told which model holds the card, and nothing here decides
anything else.

The placer has to be told because ``VramBudgetPlacer`` fit-tests every GPU-placed spawn
...
What is charged is a declared figure rather than a reading. The plan's ``brain_vram_mib`` is the
deployment's own measurement of its deep tier, and it is not taken on trust: ``swap_in``'s fit
check compares that same number against what the card reports free immediately before the load,
and fails the handoff when the card is short. So by the time the window matters, a real reading
has cleared the declared figure at the one instant such a reading is evidence. Reading the card
here instead would put an HTTP call to the sidecar inside ``place``, which is synchronous and
lock-free by design (no ``await``, so a batch of concurrent spawns races the ledger correctly);
making it async to fetch a number the swap has already checked would buy accuracy the spawn path
has no use for.

The ordering matters, since the fit check and this now read the same card. The charge is written
...
serving again, so a restore that failed and reported it leaves the handoff charge in place and
keeps spawning on the CPU, rather than admitting GPU work onto a card nothing has re-read.

None of this runs unless the deployment declared a figure. With ``brain_vram_mib`` zero (the
shipped default) there is no measured number to charge, and charging nothing would be worse: it
