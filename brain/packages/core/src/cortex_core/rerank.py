...
kept. Both landed as one widening because three deferred consumers each needed one of them.
"""
...
class RecallPolicy(Protocol):
    ...
    ``k`` (a policy that reranks needs a wider pool than it returns). ``select`` reorders and
    ...
    selection has completed (decision 8). Every heuristic policy has a synchronous body.
...
class RawRecallPolicy:
    ...
    is exactly the pre-reranking behavior. This is the default, so reranking is opt-in. Its
