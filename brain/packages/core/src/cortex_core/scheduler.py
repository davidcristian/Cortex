...
the budget's one hard wall, and the only outcome here that is not "eventually yes". It is soft in
the sense that it binds nothing it did not admit (no ``.wslconfig``/parent cgroup, the user's
constraint), yet what it *does* charge is a hard cap, because a waiting spawn holds none of the
budget. Doing no I/O, it is a pure reference impl of the ``SubagentScheduler`` port, in the core,
fully covered with no real workload.
"""
...
class ResourceBudgetScheduler:
    ...
    async def admit(self, request: PlacementRequest) -> AsyncGenerator[None, None]:
        ...
        ``SubagentAdmissionError`` instead of waiting forever; an admissible charge always
        eventually fits as peers release. That refusal is the wall, and it is deliberately the
        *only* one: a transient full budget queues, since the work will run seconds later and
        depth-1 guarantees the queue drains (ADR-0012 admission-wall addendum). ``notify_all``
        on release wakes every waiter because their asks differ. A freed slot may satisfy a
        small waiter but not a large one, so each must re-check ``_fits``.
        """
