...
design cannot leave on its own. A swap back that stopped retrying publishes that nothing is
resident (``residency_restore.py``), so every ``acquire`` raises, so no turn runs, so no handoff
starts, and the reconciliation a handoff would have run (``residency_watch.py``) is unreachable at
exactly the moment it is needed. The same dead end has a cheaper twin: a boot that could
...
It takes two readings, because a serving cortex is not on its own the standing residency. A
restore can fail at the stop as easily as at the start, so the deep model may still be holding the
card when the cortex comes back; publishing on the cortex's own state alone would then hand a lease
out onto a card that has two tiers on it, which on a 24 GB machine is how a spill or an OOM happens.
The deep tier is therefore observed off the card first, and a tier this daemon's roster never had
counts as off it, nothing being resident under a name that does not exist.

It starts nothing. A cortex that is down stays down: this observes one that is serving again,
whoever brought it back (the operator through the sidecar's control API, which the runbook's step 2
already tells them to use, or the daemon's own boot default after a restart). Starting it here
would put a whole tier load, minutes at tier scale, inside a pass that shutdown waits out, and it
would be a retry policy laid over an attempt that has already failed twice. That is a decision of
its own, and it is recorded rather than made here.
"""
...
async def heal_standing_residency(
    ...
    reading of the same machine taken at the same moment, and the resident's report is the one a
...
async def regain_residency(
    ...
    The charge is the other half of the publish and lands with it. A restore that stopped retrying
    leaves the handoff's VRAM charge in place on purpose, so that spawns overflow to the CPU rather
    than being admitted onto a card nobody can describe (``residency_charge.py``); the moment the
    card is described again, by the same reading that publishes, that reason no longer holds. It
    follows the
...
async def _deep_tier_is_off_the_card(host: ModelHost, model: str) -> bool:
    ...
    which is the same posture the peer sweep takes for the same reason: an absent reading is not a
