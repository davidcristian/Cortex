...
small model asked to mix vocabularies in one field does mix them, and a mutual-exclusion
correction states the distinction the first time it is hit.
...
# match, which is what refuses a full ISO date rather than dropping its year unreported.
...
def parse_at_time(value: object) -> tuple[int, int] | str:
    ...
    try:
        ...
        return BAD_AT_TIME
    # A rule stores hour and minute only, so accepting finer precision would drop part of what
    # the model wrote with nothing saying so, and an offset would contradict the zone it is read
    # in.
...
def parse_on_dates(value: object) -> frozenset[MonthDay] | str:
    ...
    offset, because dropping the year would answer a question the model did not ask.
