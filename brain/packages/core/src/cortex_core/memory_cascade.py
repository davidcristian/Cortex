...
(tainted or not) can spell "forget everything"
...
class SessionMemoryCascade:
    ...
    records under, so the cascade targets exactly ``write_scope(session_id)`` and cascades ONLY
    ...
    async def delete_session_memories(self, session_id: str) -> int:
        ...
        a session policy. A policy that writes to some other shared bucket (neither the session's
        own scope nor global) is likewise never swept, since the cascade runs only when the write
        scope IS the session id. Returns 0 whenever the cascade does not run.
        """
