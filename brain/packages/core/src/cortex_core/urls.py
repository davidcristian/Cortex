...
Obfuscation-resistant by construction, and deterministic and dependency-free (stdlib only). The
...
out on the same standing decision. See the ADR for why. No state and no I/O.
...
# would open a bypass nothing reports.
_NON_URL = r"\s<>\"'\)\]\}"
...
# A label of a whitespace-split host: body characters carrying no dot in any reading. That absence
# is the whole point of the rule below, so it is spelled here rather than assumed.
...
# the unit the host anchor below reads one of to establish that this is a host at all.
...
# one constrained the spelling of a separator that was present; admitting one that is absent leaves
# only what follows to carry the anchor, since `https:` and any non-space run is exactly the prose
# the fullwidth addendum protected (a fullwidth colon and `no slashes here`, which is how a
# sentence names a scheme).
#
# So the anchor asks for a host, and three shapes make one: a dotted name, which every registrable
# domain, IPv4 literal and IDN is; a bracketed literal carrying a colon, which every IPv6 literal
# is and nothing else a host can be; or a split host, whose gap is the same dot spelled with
# whitespace (ADR-0015 sixteenth addendum). A single label (`https:localhost`, `https:scheme`) is
# declined, and that is the whole false-positive budget, spent where prose lives; a bare label is
# registrable under no public suffix, so declining it costs no exfil vector. The gap costs none of
# that budget back, because a gap carries a dot token and the space between two English words
# carries none, which is what keeps `https:no slashes here` reading as the sentence it is. The dot
# counts in any reading the resolver has (`DOT_SPELLING`, the IDNA label separators and their
# references), so the CJK and entity classes reach this position too. Consumes nothing, the
# `_DATA_ANCHOR` precedent below.
_HOST_ANCHOR = (
...
# without the next delta, so a hold-back using the anchor above would release the opening one
...
# `[:]`. Reusing the opaque form is what keeps `http[:]evil.example` from being the spelling left
# out: a scheme whose authority slashes are gone reads exactly like an opaque URL, right up to the
# host that is the difference. Each family still pairs only with its own separators, so
...
# than a constant so the hold-back can use the arriving one at the same position.
...
# then a closing bracket. Broader than the refanger's literal `[.]`/`[dot]` so `URL_RE` also
# consumes a defang dot whose inner is encoded (`[&#46;]`, `[%2e]`) sitting behind a literal closing
...
# off prose (ADR-0015 seventh addendum). The separator anchors the whole match and so is matched
# before any decoding can run, but the encodings still need not be enumerated there: requiring only
# the shape of an escape leaves `normalize_url`'s decode fixpoint to resolve whichever encoding it
# actually was, so no table of encodings appears in the anchor. The marker is required: an
# unconstrained chunk here would match ordinary prose like `http(s)-only`, which strict mode would
# then redact out of the repo's own docs.
