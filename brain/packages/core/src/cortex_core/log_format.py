...
``session_id``, was written onto the record and then discarded, and three adapters had grown
their own hand-rolled second rendering into the message to work around it one line at a time.

There are two renderings, and the default is the plain one because of who reads these logs. This
is a personal, local-first assistant whose operator reads ``docker compose logs brain`` in a
terminal, and that stream is mixed: uvicorn writes its own access lines, llama.cpp writes raw
stderr, and
...
The two names are one family: how a record's fields are set down for whoever reads them, laid out
plainly beside the message or packed into one carton for transport. The alternates considered were
``plain``/``json``, rejected because it names one entry for its wire format and the other for its
lack of one, leaving no shared metaphor and nowhere for a third rendering to stand, and
``loose``/``sealed``, rejected because an operator reading an env file should not have to work out
what the pair refers to.

Both renderings withhold secrets by the same rules, in ``log_fields.py``: a field named for a
...
class UnknownLogFormatError(ValueError):
    ...
    configuration fault and a process that fell back to a default without saying so would leave an
    operator reading the wrong shape while believing they had chosen the other one.
    """
...
def configure_logging(level: int | str, *, style: str = DEFAULT_LOG_FORMAT) -> None:
    ...
    else. ``force`` is set because the entry point declares what this process logs like, and
    ``basicConfig`` without it is a no-op the moment anything has already attached a handler.
    """
