...
/// Builds the `BodyService` server over `audio` and `notifier`, fronted by the seam-token validator
/// (ADR-0016/0023): an empty `token` makes the validator a pass-through, so a tokenless deployment
/// is byte-for-byte the tokenless server.
