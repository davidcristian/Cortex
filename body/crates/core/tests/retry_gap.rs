...
//! every gap it is handed and scripts whether each wait expires, exactly as `retry.rs`'s
...
//! taken on an idle machine says nothing about a loaded one.
//!
//! Three properties are asserted: a stream that keeps producing is never cut off however long it
//! runs, a stream that stops is ended with the gap that expired, and the first event is measured
//! against one gap while every later one is measured against the other.
//!
...
//! shape keeps the suite's copies to the two that matter: this file's, and the one
...
///
/// A granted wait runs the poll, so an item that is there arrives; an expiring wait drops the
/// poll unpolled, which is what `tokio::time::timeout` does to an abandoned call and therefore
/// what an abandoned wait looks like from inside the decorator. `usize::MAX` is the sleeper that
/// never expires.
#[derive(Clone)]
struct GapSleeper {
...
async fn a_turn_that_keeps_talking_is_never_cut_off() {
    // The bound is on the silence rather than on the turn, so however many events arrive the
    // stream passes through verbatim and nothing is added.
...
async fn an_expired_gap_ends_the_stream_rather_than_waiting_again() {
    ...
    // side's decision to stop waiting, so waiting again would contradict it. The recording shows
    // it, since a decorator that polled once more would have asked for a fourth gap after the
    // third expired.
...
fn the_shipped_gaps_are_the_two_constants_and_the_idle_one_is_the_longer() {
    // The ordering is what a retune could invert by accident: the long silences on this
    // deployment (a delegated subtask waiting for admission and then running) can only happen
    // once a turn is under way, so the mid-stream gap is the loose one and the first-event gap
    // the tight one. The derivation of both is in the ADR addendum.
