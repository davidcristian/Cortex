...
/// A frame whose pixels are a deterministic function of position, so an averaged output pixel has a
/// value a test can predict.
...
/// Decodes a PNG back to `(width, height, rgb bytes)` with the decoder half of the same crate, so a
/// test never reads a dimension out of the value that produced it.
