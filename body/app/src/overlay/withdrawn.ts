...
// technology and leaves the tab order alone; worse, a browser refuses it over an ancestor of the
// focused element, so it needs the app to move focus first before it does anything at all.
// `inert` is the one that carries both halves: it takes the subtree out of the tab order, out of
// the pointer's reach and out of the accessibility tree, and it blurs whatever inside it had focus
// rather than asking to be helped. It is also newer, so `aria-hidden` stays beside it as the thing
...
// boolean attribute that is present, so the string form is not a workaround for the real thing, it
// IS the real thing, written the way the platform writes it.
//
