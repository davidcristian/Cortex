// Summoning the overlay, including the case where the activation arrives before anything is
// listening for it.
//
...
//
// So an activation is recorded as well as announced: it is stored as pending and dispatched, and
// whoever attaches next takes the pending one. Both paths clear the flag, so a later remount
// cannot replay a summon that has already been answered.
