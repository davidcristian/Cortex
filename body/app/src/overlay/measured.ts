...
//
// Nothing is rendered here. A probe that measured a hidden copy would reproduce the same defect one
// layer down, the copy and the real thing drifting apart with nobody looking at the copy. Both
...
// and is replaced by the disclosure that has to match it. So there is nothing to run at startup,
// and no startup would be early enough anyway: measured at boot, the first reading of the empty
// state is 183px against the 185px it settles at two frames later, the example chips' row coming
// out 29px before the system font stack resolves and 31px after.
//
// The empty state is therefore a reading plus a watch, the shape `PanelEdge` already uses for its
// own box: once as the element is attached, so the number is never missing, and again whenever the
// box changes. A chip is one reading, and the difference is deliberate. A chip cannot be on screen
// before the user has typed, so it is never measured through the settling the empty state sits in,
// and two of them (a tool and a status) can be up at once, which one watch per property cannot
// track: a shared ref callback is told an element is leaving but never which one. Both chips are
// the same box, so a reading apiece says the same thing and the last mount is as good as the first.
//
...
//
// When there is nothing to measure, the stylesheet's own value stands. An element with no layout
// (jsdom, a `display: none` ancestor, a node not in a document) reports 0, and an element that
// never mounts at all reports nothing: a chat restored with messages in it never shows the empty
// state, and a reply that did not reason never shows a chip. In each of those cases nothing is
// published and the value declared on `:root` in overlay.css stands, which is the same measured
// constant that used to be the only value there. The failure mode is therefore today's behaviour
// rather than a zero-height floor, which is why the constants stay in the stylesheet as documented
