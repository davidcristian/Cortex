...
// The assistant bubble that streams its reply in letter by letter (ADR-0037). Which machinery a
// message gets is latched at mount: a message already settled when this instance mounts is history
// and renders as one plain text node with none of it, while a message that was streaming keeps its
// letter DOM after settling, so nothing re-wraps or re-kerns while the reader is looking at it.
// That latch is presentation state this component holds, as the Thoughts disclosure holds its open
// state; the reducer only appends to `content`.
//
// The letter DOM is presentation rather than the accessible text: the word boxes are aria-hidden
// behind a visually hidden copy of the content, so assistive technology reads a reply rather than
// hundreds of one-letter spans, and reads the thinking label before the first token arrives.
...
/** The streaming (or once-streamed) bubble: letters, mist, and the clock that drives both. It is a
 *  separate component so that the plain-history path above runs none of these hooks. */
