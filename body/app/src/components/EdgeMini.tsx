...
// The Dream row's tile art, re-tuned for its size rather than scaled down: at a third of the real
// size the bleed becomes dead margin and the waves become a jittery line.

const WIDTH = 72;
const HEIGHT = 50;

/** The pane the liquid is drawn around inside the tile. The tail is left unscaled, so the
 *  corners' swell covers the whole small loop. */
const FRAME: LoopFrame = { x: 13, y: 9, width: 46, height: 32, radius: 7, amplitude: 0.85, tail: CORNER_TAIL };

/** Reverie's tile cycles between its two states on this period, seconds, because what defines
 *  the style is the change itself: neutral at rest, accent while a turn runs. */
const BREATHE_S = 7;

/** Phase chosen so the frozen pose (`STILL_SECONDS`) falls exactly mid-blend, so a reduced-motion
...
export function EdgeMini({ style, idPrefix, animated }: EdgeMiniProps) {
  ...
  return (
    ...
    >
      ...
      {/* No ground is painted here: the tile itself is the ground the small window stands on, and
          painting one read as a panel stuck onto the swatch. The theme tiles do paint one, because
          there the ground is what is being chosen. */}
